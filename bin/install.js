...
const WTDD_CODEX_MARKER = '# WTDD Agent Configuration \u2014 managed by what-the-dog-doing installer';
const WTDD_CODEX_HOOKS_OWNERSHIP_PREFIX = '# WTDD codex_hooks ownership: ';

// Copilot instructions marker constants
const WTDD_COPILOT_INSTRUCTIONS_MARKER = '<!-- WTDD Configuration \u2014 managed by what-the-dog-doing installer -->';
const WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /WTDD Configuration -->';

const CODEX_AGENT_SANDBOX = {
  'wtdd-executor': 'workspace-write',
  'wtdd-planner': 'workspace-write',
  'wtdd-phase-researcher': 'workspace-write',
  'wtdd-project-researcher': 'workspace-write',
  'wtdd-research-synthesizer': 'workspace-write',
  'wtdd-verifier': 'workspace-write',
  'wtdd-codebase-mapper': 'workspace-write',
  'wtdd-roadmapper': 'workspace-write',
  'wtdd-debugger': 'workspace-write',
  'wtdd-plan-checker': 'read-only',
  'wtdd-integration-checker': 'read-only',
};
...
Then re-run: npx what-the-dog-doing-cc@latest
`);
    process.exit(1);
  }
}
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx what-the-dog-doing-cc [options]\n\n  ${yellow}Options:${reset}\n    ${cyan}-g, --global${reset}              Install globally (to config directory)\n    ${cyan}-l, --local${reset}               Install locally (to current directory)\n    ${cyan}--claude${reset}                  Install for Claude Code only\n    ${cyan}--opencode${reset}                Install for OpenCode only\n    ${cyan}--gemini${reset}                  Install for Gemini only\n    ${cyan}--kilo${reset}                    Install for Kilo only\n    ${cyan}--codex${reset}                   Install for Codex only\n    ${cyan}--copilot${reset}                 Install for Copilot only\n    ${cyan}--antigravity${reset}             Install for Antigravity only\n    ${cyan}--cursor${reset}                  Install for Cursor only\n    ${cyan}--windsurf${reset}                Install for Windsurf only\n    ${cyan}--augment${reset}                 Install for Augment only\n    ${cyan}--trae${reset}                    Install for Trae only\n    ${cyan}--all${reset}                     Install for all runtimes\n    ${cyan}-u, --uninstall${reset}           Uninstall WTDD (remove all WTDD files)\n    ${cyan}-c, --config-dir <path>${reset}   Specify custom config directory\n    ${cyan}-h, --help${reset}                Show this help message\n    ${cyan}--force-statusline${reset}        Replace existing statusline config\n\n  ${yellow}Examples:${reset}\n    ${dim}# Interactive install (prompts for runtime and location)${reset}\n    npx what-the-dog-doing-cc\n\n    ${dim}# Install for Claude Code globally${reset}\n    npx what-the-dog-doing-cc --claude --global\n\n    ${dim}# Install for Gemini globally${reset}\n    npx what-the-dog-doing-cc --gemini --global\n\n    ${dim}# Install for Kilo globally${reset}\n    npx what-the-dog-doing-cc --kilo --global\n\n    ${dim}# Install for Codex globally${reset}\n    npx what-the-dog-doing-cc --codex --global\n\n    ${dim}# Install for Copilot globally${reset}\n    npx what-the-dog-doing-cc --copilot --global\n\n    ${dim}# Install for Copilot locally${reset}\n    npx what-the-dog-doing-cc --copilot --local\n\n    ${dim}# Install for Antigravity globally${reset}\n    npx what-the-dog-doing-cc --antigravity --global\n\n    ${dim}# Install for Antigravity locally${reset}\n    npx what-the-dog-doing-cc --antigravity --local\n\n    ${dim}# Install for Cursor globally${reset}\n    npx what-the-dog-doing-cc --cursor --global\n\n    ${dim}# Install for Cursor locally${reset}\n    npx what-the-dog-doing-cc --cursor --local\n\n    ${dim}# Install for Windsurf globally${reset}\n    npx what-the-dog-doing-cc --windsurf --global\n\n    ${dim}# Install for Windsurf locally${reset}\n    npx what-the-dog-doing-cc --windsurf --local\n\n    ${dim}# Install for Augment globally${reset}\n    npx what-the-dog-doing-cc --augment --global\n\n    ${dim}# Install for Augment locally${reset}\n    npx what-the-dog-doing-cc --augment --local\n\n    ${dim}# Install for Trae globally${reset}\n    npx what-the-dog-doing-cc --trae --global\n\n    ${dim}# Install for Trae locally${reset}\n    npx what-the-dog-doing-cc --trae --local\n\n    ${dim}# Install for all runtimes globally${reset}\n    npx what-the-dog-doing-cc --all --global\n\n    ${dim}# Install to custom config directory${reset}\n    npx what-the-dog-doing-cc --kilo --global --config-dir ~/.kilo-work\n\n    ${dim}# Install to current project only${reset}\n    npx what-the-dog-doing-cc --claude --local\n\n    ${dim}# Uninstall WTDD from Cursor globally${reset}\n    npx what-the-dog-doing-cc --cursor --global --uninstall\n\n  ${yellow}Notes:${reset}\n    The --config-dir option is useful when you have multiple configurations.\n    It takes priority over CLAUDE_CONFIG_DIR / OPENCODE_CONFIG_DIR / GEMINI_CONFIG_DIR / KILO_CONFIG_DIR / CODEX_HOME / COPILOT_CONFIG_DIR / ANTIGRAVITY_CONFIG_DIR / CURSOR_CONFIG_DIR / WINDSURF_CONFIG_DIR / AUGMENT_CONFIG_DIR / TRAE_CONFIG_DIR environment variables.\n`);
  process.exit(0);
}
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all wtdd: references → wtdd-)
  c = c.replace(/wtdd:/g, 'wtdd-');
 ...
 * convert name from wtdd:xxx to wtdd-xxx format.
 */
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all wtdd: references → wtdd-)
  c = c.replace(/wtdd:/g, 'wtdd-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize wtdd: -> wtdd-.
  // This preserves rendered "next step" commands like "/wtdd-execute-phase 17".
  return content.replace(/wtdd:/gi, 'wtdd-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{WTDD_ARGS}}');
...
- Treat all user text after the skill mention as \`{{WTDD_ARGS}}\`.
- If no arguments are present, treat \`{{WTDD_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing WTDD workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run WTDD workflow ${skillName}.`;
  if (frontmatter) {
...
## C. Tool Usage
Use these Windsurf tools when executing WTDD workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
## C. Tool Usage
Use these Augment tools when executing WTDD workflows:
...
function copyCommandsAsAugmentSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous WTDD Augment skills to avoid stale command skills
...
function convertSlashCommandsToTraeSkillMentions(content) {
  return content.replace(/\/wtdd:([a-z0-9-]+)/g, (_, commandName) => {
    return `/wtdd-${commandName}`;
  });
}
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/wtdd:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$wtdd-${String(commandName).toLowerCase()}`;
  });
  converted = converted.replace(/\/wtdd-help\b/g, '$wtdd-help');
  return converted;
}
...
- Treat all user text after \`${invocation}\` as \`{{WTDD_ARGS}}\`.
- If no arguments are present, treat \`{{WTDD_ARGS}}\` as empty.

## B. AskUserQuestion → request_user_input Mapping
WTDD workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
WTDD workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:

Direct mapping:
- \`Task(subagent_type="X", prompt="Y")\` → \`spawn_agent(agent_type="X", message="Y")\`
- \`Task(model="...")\` → omit (Codex uses per-role config, not inline model selection)
- \`fork_context: false\` by default — WTDD agents load their own context via \`<files_to_read>\` blocks
...
function generateCodexAgentToml(agentName, agentContent) {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `WTDD agent ${resolvedName}`
  );
...
/**
 * Generate the WTDD config block for Codex config.toml.
...
function stripCodexGsdAgentSections(content) {
  return content.replace(/^\[agents\.wtdd-[^\]]+\]\n(?:(?!\[)[^\n]*\n?)*/gm, '');
}

/**
 * Strip WTDD sections from Codex config.toml content.
 * Returns cleaned content, or null if file would be empty.
 */
function stripGsdFromCodexConfig(content) {
  const eol = detectLineEnding(content);
  const markerIndex = content.indexOf(WTDD_CODEX_MARKER);
  const codexHooksOwnership = getManagedCodexHooksOwnership(content);

  if (markerIndex !== -1) {
    // Has WTDD marker — remove everything from marker to EOF
    let before = content.substring(0, markerIndex);
    before = stripCodexHooksFeatureAssignments(before, codexHooksOwnership);
    // Also strip WTDD-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have WTDD-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.wtdd-*] sections (from header to next section or EOF)
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + WTDD_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# WTDD codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + WTDD_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${WTDD_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}
...
function stripLeakedGsdCodexSections(content) {
  const leakedSections = getTomlTableSections(content)
    .filter((section) =>
      section.path.startsWith('agents.wtdd-') ||
      (
        section.path === 'agents' &&
...
/**
 * Merge WTDD config block into an existing or new config.toml.
 * Three cases: new file, existing with WTDD marker, existing without marker.
 */
function mergeCodexConfig(configPath, wtddBlock) {
  // Case 1: No config.toml — create fresh
  if (!fs.existsSync(configPath)) {
    fs.writeFileSync(configPath, wtddBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(configPath, 'utf8');
  const eol = detectLineEnding(existing);
  const normalizedGsdBlock = wtddBlock.replace(/\r?\n/g, eol);
  const markerIndex = existing.indexOf(WTDD_CODEX_MARKER);

  // Case 2: Has WTDD marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any WTDD-managed sections that leaked above the marker from previous installs
  ...
  // Case 3: No marker — append WTDD block
...
/**
 * Repair config.toml files corrupted by pre-#1346 WTDD installs.
...
/**
 * Merge WTDD instructions into copilot-instructions.md.
 ...
 * @param {string} wtddContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, wtddContent) {
  const wtddBlock = WTDD_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    wtddContent.trim() + '\n' +
    WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, wtddBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(WTDD_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has WTDD markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
    if (before) newContent += before + '\n\n';
    newContent += wtddBlock;
    if (after) newContent += '\n\n' + after;
    newContent += '\n';
...
/**
 * Strip WTDD section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was WTDD-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripGsdFromCopilotInstructions(content) {
  const openIndex = content.indexOf(WTDD_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('wtdd-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex WTDD install path (absolute, so subagents with empty $HOME work — #820)
  const codexGsdPath = `${path.resolve(targetDir, 'what-the-dog-doing').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/what-the-dog-doing prefix so path resolves to codex WTDD install
    content = content.replace(/~\/\.claude\/what-the-dog-doing\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/what-the-dog-doing\//g, codexGsdPath);
  ...
  const wtddBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, wtddBlock);
...
function convertClaudeToGeminiAgent(content) {
  ...
  // when they can't be resolved. WTDD agents use ${PHASE}, ${PLAN}, etc. as
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /wtdd-command colon variant with /wtdd-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/wtdd:/g, '/wtdd-');
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false } = {}) {
  ...
  convertedContent = convertedContent.replace(/\bSlashCommand\b/g, 'skill');
  convertedContent = convertedContent.replace(/\bTodoWrite\b/g, 'todowrite');
  // Replace /wtdd-command colon variant with /wtdd-command for Kilo (flat command structure)
  convertedContent = convertedContent.replace(/\/wtdd:/g, '/wtdd-');
 ...
 * OpenCode expects: command/wtdd-help.md (invoked as /wtdd-help)
 * Source structure: commands/wtdd/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/wtdd/)
 * @param {string} destDir - Destination directory (e.g., command/)
 * @param {string} prefix - Prefix for filenames (e.g., 'wtdd')
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old wtdd-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/wtdd/debug/start.md -> command/wtdd-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> wtdd-help.md
      const baseName = entry.name.replace('.md', '');
...
function listCodexSkillNames(skillsDir, prefix = 'wtdd-') {
...
function copyCommandsAsCodexSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous WTDD Codex skills to avoid stale command skills.
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCursorSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous WTDD Cursor skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsWindsurfSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous WTDD Windsurf skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCopilotSkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous WTDD Copilot skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
 ...
 * Claude Code 2.1.88+ uses skills/xxx/SKILL.md instead of commands/wtdd/xxx.md.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'wtdd')
 * @param {string} pathPrefix - Path prefix for file references
...
function copyCommandsAsClaudeSkills(srcDir, skillsDir, prefix, pathPrefix, runtime, isGlobal = false) {
  ...
  // Remove previous WTDD Claude skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
/**
 * Recursively install WTDD commands as Antigravity skills.
...
function copyCommandsAsAntigravitySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous WTDD Antigravity skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/wtdd:/gi, 'wtdd-');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\/wtdd:([a-z0-9-]+)/g, (_, commandName) => {
        return `/wtdd-${commandName}`;
      });
...
/**
 * Clean up orphaned files from previous WTDD versions
 */
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/wtdd-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to wtdd-statusline.js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'wtdd-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to wtdd-statusline.js in v1.9.0
    'wtdd-intel-index.js',  // Removed in v1.9.2
    'wtdd-intel-session.js',  // Removed in v1.9.2
    'wtdd-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old WTDD statusline.js path
  // Only match the specific old WTDD path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1wtdd-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/wtdd-statusline.js)`);
  }
...
/**
 * Uninstall WTDD from the specified directory for a specific runtime
 * Removes only WTDD-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling WTDD from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 1. Remove WTDD commands/skills
  if (isOpencode || isKilo) {
    // OpenCode/Kilo: remove command/wtdd-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if (file.startsWith('wtdd-') && file.endsWith('.md')) {
          fs.unlinkSync(path.join(commandDir, file));
          removedCount++;
        }
      }
      console.log(`  ${green}✓${reset} Removed WTDD commands from command/`);
    }
  } else if (isCodex || isCursor || isWindsurf || isTrae) {
    // Codex/Cursor/Windsurf/Trae: remove skills/wtdd-*/SKILL.md skill directories
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name.startsWith('wtdd-')) {
          fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
          skillCount++;
        }
      }
    ...
    // Codex-only: remove WTDD agent .toml config files and config.toml sections
    if (isCodex) {
      ...
      if (fs.existsSync(codexAgentsDir)) {
        ...
        for (const file of tomlFiles) {
          if (file.startsWith('wtdd-') && file.endsWith('.toml')) {
      ...
      // Codex: clean WTDD sections from config.toml
      const configPath = path.join(targetDir, 'config.toml');
      if (fs.existsSync(configPath)) {
        ...
        if (cleaned === null) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed config.toml (was WTDD-only)`);
        } else if (cleaned !== content) {
          fs.writeFileSync(configPath, cleaned);
          removedCount++;
          console.log(`  ${green}✓${reset} Cleaned WTDD sections from config.toml`);
        }
      }
    }
  } else if (isCopilot) {
    // Copilot: remove skills/wtdd-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Copilot: clean WTDD section from copilot-instructions.md
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was WTDD-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned WTDD section from copilot-instructions.md`);
      }
    }
  } else if (isAntigravity) {
    // Antigravity: remove skills/wtdd-*/ directories (same layout as Copilot skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Gemini: still uses commands/wtdd/
    const wtddCommandsDir = path.join(targetDir, 'commands', 'wtdd');
    if (fs.existsSync(wtddCommandsDir)) {
      ...
      const devPrefsPath = path.join(wtddCommandsDir, 'dev-preferences.md');
      const preservedDevPrefs = fs.existsSync(devPrefsPath) ? fs.readFileSync(devPrefsPath, 'utf-8') : null;

      fs.rmSync(wtddCommandsDir, { recursive: true });
      removedCount++;
      console.log(`  ${green}✓${reset} Removed commands/wtdd/`);

      // Restore user-generated files
      if (preservedDevPrefs) {
        try {
          fs.mkdirSync(wtddCommandsDir, { recursive: true });
          fs.writeFileSync(devPrefsPath, preservedDevPrefs);
          console.log(`  ${green}✓${reset} Preserved commands/wtdd/dev-preferences.md`);
        } catch (err) {
    ...
    // Claude Code global: remove skills/wtdd-*/ directories (primary global install location)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Also clean up legacy commands/wtdd/ from older global installs
    const legacyCommandsDir = path.join(targetDir, 'commands', 'wtdd');
    if (fs.existsSync(legacyCommandsDir)) {
      ...
      removedCount++;
      console.log(`  ${green}✓${reset} Removed legacy commands/wtdd/`);
    ...
    // Claude Code local: remove commands/wtdd/ (primary local install location since #1736)
    const wtddCommandsDir = path.join(targetDir, 'commands', 'wtdd');
    if (fs.existsSync(wtddCommandsDir)) {
      // Preserve user-generated files before wipe (#1423)
      const devPrefsPath = path.join(wtddCommandsDir, 'dev-preferences.md');
  ...
  // 2. Remove what-the-dog-doing directory
  const wtddDir = path.join(targetDir, 'what-the-dog-doing');
  if (fs.existsSync(wtddDir)) {
    // Preserve user-generated files before wipe (#1423)
    const userProfilePath = path.join(wtddDir, 'USER-PROFILE.md');
    const preservedProfile = fs.existsSync(userProfilePath) ? fs.readFileSync(userProfilePath, 'utf-8') : null;

    fs.rmSync(wtddDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed what-the-dog-doing/`);
    ...
    if (preservedProfile) {
      try {
        ...
        console.log(`  ${green}✓${reset} Preserved what-the-dog-doing/USER-PROFILE.md`);
      } catch (err) {
  ...
  // 3. Remove WTDD agents (wtdd-*.md files only)
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} WTDD agents`);
    }
  }
  ...
  if (fs.existsSync(hooksDir)) {
    const wtddHooks = ['wtdd-statusline.js', 'wtdd-check-update.js', 'wtdd-context-monitor.js', 'wtdd-prompt-guard.js', 'wtdd-read-guard.js', 'wtdd-workflow-guard.js', 'wtdd-session-state.sh', 'wtdd-validate-commit.sh', 'wtdd-phase-boundary.sh'];
    let hookCount = 0;
    for (const hook of wtddHooks) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} WTDD hooks`);
    }
  }

  // 5. Remove WTDD package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed WTDD package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove WTDD hooks and statusline)
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    // Remove WTDD statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
      settings.statusLine.command.includes('wtdd-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed WTDD statusline from settings`);
    }

    // Remove WTDD hooks from settings — per-hook granularity to preserve
    // user hooks that share an entry with a WTDD hook (#1755 followup)
      ...
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName]
          .map(entry => {
            ...
            // Filter out individual WTDD hooks, keep user hooks
    ...
    if (settingsModified) {
      console.log(`  ${green}✓${reset} Removed WTDD hooks from settings`);
    }
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove WTDD permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('what-the-dog-doing')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed WTDD permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No WTDD files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} WTDD has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading WTDD reference docs
 * This prevents permission prompts when WTDD accesses the what-the-dog-doing directory
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  // Build the WTDD path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const wtddPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/what-the-dog-doing/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/what-the-dog-doing/*`;
  ...
  if (config.permission.read[wtddPath] !== 'allow') {
    config.permission.read[wtddPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[wtddPath] !== 'allow') {
    config.permission.external_directory[wtddPath] = 'allow';
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for WTDD docs`);
}

/**
 * Configure Kilo permissions to allow reading WTDD reference docs
 * This prevents permission prompts when WTDD accesses the what-the-dog-doing directory
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureKiloPermissions(isGlobal = true, configDir = null) {
  ...
  const wtddPath = kiloConfigDir === defaultConfigDir
    ? '~/.config/kilo/what-the-dog-doing/*'
    : `${kiloConfigDir.replace(/\\/g, '/')}/what-the-dog-doing/*`;
...
function writeManifest(configDir, runtime = 'claude') {
  ...
  const isWindsurf = runtime === 'windsurf';
  const isTrae = runtime === 'trae';
  const wtddDir = path.join(configDir, 'what-the-dog-doing');
  const commandsDir = path.join(configDir, 'commands', 'wtdd');
  const opencodeCommandDir = path.join(configDir, 'command');
  const codexSkillsDir = path.join(configDir, 'skills');
  const agentsDir = path.join(configDir, 'agents');
  const manifest = { version: pkg.version, timestamp: new Date().toISOString(), files: {} };

  const wtddHashes = generateManifest(wtddDir);
  for (const [rel, hash] of Object.entries(wtddHashes)) {
    manifest.files['what-the-dog-doing/' + rel] = hash;
  }
  if (isGemini && fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/wtdd/' + rel] = hash;
    }
  }
  ...
  if (!isCodex && !isCopilot) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('wtdd-') && (file.endsWith('.js') || file.endsWith('.sh'))) {
...
/**
 * Detect user-modified WTDD files by comparing against install manifest.
 * Backs up modified files to wtdd-local-patches/ for reapply after update.
 * Also saves pristine copies (from manifest) to wtdd-pristine/ to enable
...
function saveLocalPatches(configDir) {
  ...
  const pristineDir = path.join(configDir, 'wtdd-pristine');
  const modified = [];
  ...
  // These represent the original WTDD distribution files that the user then modified.
  ...
  if (modified.length > 0) {
    ...
    // knows which WTDD version these files came from, enabling npm-based reconstruction.
    const meta = {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified WTDD file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
...
function reportLocalPatches(configDir, runtime = 'claude') {
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'kilo' || runtime === 'copilot')
      ? '/wtdd-reapply-patches'
      : runtime === 'codex'
        ? '$wtdd-reapply-patches'
        : runtime === 'cursor'
          ? 'wtdd-reapply-patches (mention the skill name)'
          : '/wtdd-reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Path prefix for file references in markdown content (e.g. wtdd-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>what-the-dog-doing/bin/...
  ...
  // Save any locally modified WTDD files before they get wiped
  saveLocalPatches(targetDir);

  // Clean up orphaned files from previous versions
  cleanupOrphanedFiles(targetDir);

  // OpenCode/Kilo use command/ (flat), Codex uses skills/, Claude/Gemini use commands/wtdd/
  if (isOpencode || isKilo) {
    // OpenCode/Kilo: flat structure in command/ directory
    const commandDir = path.join(targetDir, 'command');
    fs.mkdirSync(commandDir, { recursive: true });

    // Copy commands/wtdd/*.md as command/wtdd-*.md (flatten structure)
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyFlattenedCommands(wtddSrc, commandDir, 'wtdd', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/wtdd-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('wtdd-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/wtdd-*');
    }
  } else if (isCodex) {
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsCodexSkills(wtddSrc, skillsDir, 'wtdd', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
      failures.push('skills/wtdd-*');
    }
  } else if (isCopilot) {
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsCopilotSkills(wtddSrc, skillsDir, 'wtdd', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('wtdd-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsAntigravitySkills(wtddSrc, skillsDir, 'wtdd', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('wtdd-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsCursorSkills(wtddSrc, skillsDir, 'wtdd', pathPrefix, runtime);
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsWindsurfSkills(wtddSrc, skillsDir, 'wtdd', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir); // reuse — same dir structure
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsAugmentSkills(wtddSrc, skillsDir, 'wtdd', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsTraeSkills(wtddSrc, skillsDir, 'wtdd', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    const wtddDest = path.join(commandsDir, 'wtdd');
    copyWithPathReplacement(wtddSrc, wtddDest, pathPrefix, runtime, true, isGlobal);
    if (verifyInstalled(wtddDest, 'commands/wtdd')) {
      console.log(`  ${green}✓${reset} Installed commands/wtdd`);
    } else {
      failures.push('commands/wtdd');
    }
  } else if (isGlobal) {
    // Claude Code global: skills/ format (2.1.88+ compatibility)
    const skillsDir = path.join(targetDir, 'skills');
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    copyCommandsAsClaudeSkills(wtddSrc, skillsDir, 'wtdd', pathPrefix, runtime, isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('wtdd-')).length;
      if (count > 0) {
    ...
    // Clean up legacy commands/wtdd/ from previous global installs
    const legacyCommandsDir = path.join(targetDir, 'commands', 'wtdd');
    if (fs.existsSync(legacyCommandsDir)) {
      fs.rmSync(legacyCommandsDir, { recursive: true });
      console.log(`  ${green}✓${reset} Removed legacy commands/wtdd/ directory`);
    }
  } else {
    // Claude Code local: commands/wtdd/ format — Claude Code reads local project
    // commands from .claude/commands/wtdd/, not .claude/skills/
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const wtddSrc = path.join(src, 'commands', 'wtdd');
    const wtddDest = path.join(commandsDir, 'wtdd');
    copyWithPathReplacement(wtddSrc, wtddDest, pathPrefix, runtime, true, isGlobal);
    if (verifyInstalled(wtddDest, 'commands/wtdd')) {
      const count = fs.readdirSync(wtddDest).filter(f => f.endsWith('.md')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to commands/wtdd/`);
    } else {
      failures.push('commands/wtdd');
    }
    ...
    if (fs.existsSync(staleSkillsDir)) {
      ...
      if (staleGsd.length > 0) {
        console.log(`  ${green}✓${reset} Removed ${staleGsd.length} stale WTDD skill(s) from skills/`);
      }
    }
  }

  // Copy what-the-dog-doing skill with path replacement
  const skillSrc = path.join(src, 'what-the-dog-doing');
  const skillDest = path.join(targetDir, 'what-the-dog-doing');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime, false, isGlobal);
  if (verifyInstalled(skillDest, 'what-the-dog-doing')) {
    console.log(`  ${green}✓${reset} Installed what-the-dog-doing`);
  } else {
    failures.push('what-the-dog-doing');
  }

  // Copy agents to agents directory
  const agentsSrc = path.join(src, 'agents');
  const agentsSrcPath = fs.existsSync(path.join(agentsSrc, 'wtdd')) ? path.join(agentsSrc, 'wtdd') : agentsSrc;
  if (fs.existsSync(agentsSrc)) {
    const agentsDest = path.join(targetDir, 'agents');
    fs.mkdirSync(agentsDest, { recursive: true });

    // Remove old WTDD agents (wtdd-*.md) before copying new ones
    ...
    // Copy new agents
    const agentEntries = fs.readdirSync(agentsSrcPath, { withFileTypes: true });
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        let content = fs.readFileSync(path.join(agentsSrcPath, entry.name), 'utf8');
  ...
  const changelogDest = path.join(targetDir, 'what-the-dog-doing', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'what-the-dog-doing', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae) {
    // Write package.json to force CommonJS mode for WTDD scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // and stamp the current WTDD version into the hook version header
          if (entry.endsWith('.js')) {
            let content = fs.readFileSync(srcFile, 'utf8');
            content = content.replace(/'\.claude'/g, configDirReplacement);
            content = content.replace(/\{\{WTDD_VERSION\}\}/g, pkg.version);
      ...
      if (verifyInstalled(hooksDest, 'hooks')) {
        ...
        const expectedShHooks = ['wtdd-session-state.sh', 'wtdd-validate-commit.sh', 'wtdd-phase-boundary.sh'];
  ...
  // Cache lives at ~/.cache/wtdd/ (see hooks/wtdd-check-update.js line 35-36)
  const updateCacheFile = path.join(os.homedir(), '.cache', 'wtdd', 'wtdd-update-check.json');
  ...
  if (isCodex) {
    ...
    const agentCount = installCodexConfig(targetDir, agentsSrcPath);
    ...
    try {
      ...
      const updateCheckScript = path.resolve(targetDir, 'hooks', 'wtdd-check-update.js').replace(/\\/g, '/');
      const hookBlock =
        `${eol}# WTDD Hooks${eol}` +
        `[[hooks]]${eol}` +
      ...
      // Migrate legacy wtdd-update-check entries from prior installs (#1755 followup)
      // Remove stale hook blocks that used the inverted filename or wrong path
      if (configContent.includes('wtdd-update-check')) {
        configContent = configContent.replace(/\n# WTDD Hooks\n\[\[hooks\]\]\nevent = "SessionStart"\ncommand = "node [^\n]*wtdd-update-check\.js"\n/g, '\n');
        configContent = configContent.replace(/\r\n# WTDD Hooks\r\n\[\[hooks\]\]\r\nevent = "SessionStart"\r\ncommand = "node [^\r\n]*wtdd-update-check\.js"\r\n/g, '\r\n');
      }

      if (hasEnabledCodexHooksFeature(configContent) && !configContent.includes('wtdd-check-update')) {
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'what-the-dog-doing', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'wtdd-statusline.js')
    : 'node ' + dirName + '/hooks/wtdd-statusline.js';
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'wtdd-check-update.js')
    : 'node ' + dirName + '/hooks/wtdd-check-update.js';
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'wtdd-context-monitor.js')
    : 'node ' + dirName + '/hooks/wtdd-context-monitor.js';
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'wtdd-prompt-guard.js')
    : 'node ' + dirName + '/hooks/wtdd-prompt-guard.js';
  const readGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'wtdd-read-guard.js')
    : 'node ' + dirName + '/hooks/wtdd-read-guard.js';
  ...
  if (!isOpencode && !isKilo) {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-check-update'))
    );
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook) {
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (h.command && h.command.includes('wtdd-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-prompt-guard'))
    );
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-read-guard'))
    );
    ...
    // Detects file edits outside WTDD workflow context and advises using
    // /wtdd-quick or /wtdd-fast for state-tracked changes. Advisory only.
    const workflowGuardCommand = isGlobal
      ? buildHookCommand(targetDir, 'wtdd-workflow-guard.js')
      : 'node ' + dirName + '/hooks/wtdd-workflow-guard.js';
    const hasWorkflowGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-workflow-guard'))
    );
    ...
    const validateCommitCommand = isGlobal
      ? 'bash ' + targetDir.replace(/\\/g, '/') + '/hooks/wtdd-validate-commit.sh'
      : 'bash ' + dirName + '/hooks/wtdd-validate-commit.sh';
    const hasValidateCommitHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-validate-commit'))
    );
    ...
    const validateCommitFile = path.join(targetDir, 'hooks', 'wtdd-validate-commit.sh');
    if (!hasValidateCommitHook && fs.existsSync(validateCommitFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped commit validation hook — wtdd-validate-commit.sh not found at target`);
    }

    // Configure session state orientation hook (opt-in)
    const sessionStateCommand = isGlobal
      ? 'bash ' + targetDir.replace(/\\/g, '/') + '/hooks/wtdd-session-state.sh'
      : 'bash ' + dirName + '/hooks/wtdd-session-state.sh';
    const hasSessionStateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-session-state'))
    );
    const sessionStateFile = path.join(targetDir, 'hooks', 'wtdd-session-state.sh');
    if (!hasSessionStateHook && fs.existsSync(sessionStateFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped session state hook — wtdd-session-state.sh not found at target`);
    }

    // Configure phase boundary detection hook (opt-in)
    const phaseBoundaryCommand = isGlobal
      ? 'bash ' + targetDir.replace(/\\/g, '/') + '/hooks/wtdd-phase-boundary.sh'
      : 'bash ' + dirName + '/hooks/wtdd-phase-boundary.sh';
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('wtdd-phase-boundary'))
    );
    const phaseBoundaryFile = path.join(targetDir, 'hooks', 'wtdd-phase-boundary.sh');
    if (!hasPhaseBoundaryHook && fs.existsSync(phaseBoundaryFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped phase boundary hook — wtdd-phase-boundary.sh not found at target`);
    }
  }
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null) {
  ...
  // For non-Claude runtimes, set resolve_model_ids: "omit" in ~/.wtdd/defaults.json
  ...
  // See #1156.
  if (runtime !== 'claude') {
    const wtddDir = path.join(os.homedir(), '.wtdd');
    const defaultsPath = path.join(wtddDir, 'defaults.json');
    try {
      fs.mkdirSync(wtddDir, { recursive: true });
      let defaults = {};
      ...
      if (defaults.resolve_model_ids !== 'omit') {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.wtdd/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.wtdd/defaults.json: ${e.message}`);
    }
  }
  ...
  let command = '/wtdd-new-project';
  if (runtime === 'opencode') command = '/wtdd-new-project';
  if (runtime === 'kilo') command = '/wtdd-new-project';
  if (runtime === 'codex') command = '$wtdd-new-project';
  if (runtime === 'copilot') command = '/wtdd-new-project';
  if (runtime === 'antigravity') command = '/wtdd-new-project';
  if (runtime === 'cursor') command = 'wtdd-new-project (mention the skill name)';
  if (runtime === 'windsurf') command = '/wtdd-new-project';
  if (runtime === 'augment') command = '/wtdd-new-project';
  if (runtime === 'trae') command = '/wtdd-new-project';
  console.log(`
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  WTDD includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with WTDD statusline
`);
...
/**
 * Install WTDD for all selected runtimes
 */
...
if (process.env.WTDD_TEST_MODE) {
  module.exports = {
    ...
    WTDD_COPILOT_INSTRUCTIONS_MARKER,
    WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER,
...
} // end of else block for WTDD_TEST_MODE
