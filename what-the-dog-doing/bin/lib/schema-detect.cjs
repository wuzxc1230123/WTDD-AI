...
function checkSchemaDrift(changedFiles, executionLog, options = {}) {
  ...
  const message = [
    ...
    pushCommands,
    '',
    'Run the appropriate push command, or set WTDD_SKIP_SCHEMA_CHECK=true to bypass this gate.',
  ].join('\n');
  ...
  if (skipCheck) {
    return {
      ...
      orms: detection.orms,
      unpushedOrms,
      message: 'Schema drift detected but check was skipped (WTDD_SKIP_SCHEMA_CHECK=true).',
    };
  }
