 ...
 *   2. User-level defaults from ~/.wtdd/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /wtdd-new-project
 *
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.wtdd/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.wtdd', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 * configured during /wtdd-new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.wtdd/defaults.json.
 *
