...
/**
 * Remove stale wtdd-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'wtdd-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'wtdd-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `wtdd-${Date.now()}.json`);
...
const CONFIG_DEFAULTS = {
  ...
  phase_branch_template: 'wtdd/phase-{phase}-{slug}',
  milestone_branch_template: 'wtdd/{milestone}-{slug}',
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      process.stderr.write(
        `wtdd-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
      );
    }
    ...
    // Fall back to ~/.wtdd/defaults.json only for truly pre-project contexts (#1683)
    // If .planning/ exists, the project is initialized — just missing config.json
    if (fs.existsSync(planningDir(cwd))) {
      return defaults;
    }
    try {
      const home = process.env.WTDD_HOME || os.homedir();
      const globalDefaultsPath = path.join(home, '.wtdd', 'defaults.json');
 ...
 * Applied at write points so WTDD-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 *
 * Resolution order:
 * 1. If WTDD_PROJECT is set (env var or explicit `project` arg), routes to
 ...
 * 2. If WTDD_WORKSTREAM is set, routes to `.planning/workstreams/{ws}/`.
 * 3. Otherwise returns `.planning/`.
 *
 * WTDD_PROJECT and WTDD_WORKSTREAM can be combined:
 *   `.planning/{project}/workstreams/{ws}/`
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks WTDD_WORKSTREAM env var
 * @param {string} [project] - explicit project name; if omitted, checks WTDD_PROJECT env var
 */
function planningDir(cwd, ws, project) {
  if (project === undefined) project = process.env.WTDD_PROJECT || null;
  if (ws === undefined) ws = process.env.WTDD_WORKSTREAM || null;

  // Reject path separators and traversal components in project/workstream names
  const BAD_SEGMENT = /[/\\]|\.\./;
  if (project && BAD_SEGMENT.test(project)) {
    throw new Error(`WTDD_PROJECT contains invalid path characters: ${project}`);
  }
  if (ws && BAD_SEGMENT.test(ws)) {
    throw new Error(`WTDD_WORKSTREAM contains invalid path characters: ${ws}`);
  }
 ...
 *
 * Order:
 * 1. Explicit runtime/session env vars (`WTDD_SESSION_KEY`, `CODEX_THREAD_ID`, etc.)
...
function getSessionScopedWorkstreamFile(cwd) {
  ...
  const dirPath = path.join(os.tmpdir(), 'wtdd-workstream-sessions', projectId);
  return {
    sessionKey,
    dirPath,
...
/**
 * Resolve the agents directory from the WTDD install location.
 * wtdd-tools.cjs lives at <configDir>/what-the-dog-doing/bin/wtdd-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * WTDD_AGENTS_DIR env var overrides the default path. Used in tests and for
 * installs where the agents directory is not co-located with wtdd-tools.cjs.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  if (process.env.WTDD_AGENTS_DIR) {
    return process.env.WTDD_AGENTS_DIR;
  }
  // __dirname is what-the-dog-doing/bin/lib/ → go up 3 levels to configDir
  return path.join(__dirname, '..', '..', '..', 'agents', 'wtdd');
}

/**
 * Check which WTDD agents are installed on disk.
 * Returns an object with installation status and details.
 *
 * Recognises both standard format (wtdd-planner.md) and Copilot format
 * (wtdd-planner.agent.md). Copilot renames agent files during install (#1512).
 *
