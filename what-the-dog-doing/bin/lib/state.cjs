...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
  ...
  process.stderr.write(
    `[wtdd-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { wtdd_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when WTDD hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .wtdd/WAITING.json if .wtdd exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const wtddDir = fs.existsSync(path.join(cwd, '.wtdd')) ? path.join(cwd, '.wtdd') : planningDir(cwd);
  const waitingPath = path.join(wtddDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.wtdd', 'WAITING.json'),
