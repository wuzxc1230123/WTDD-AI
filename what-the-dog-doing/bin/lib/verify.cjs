...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (!fs.existsSync(planBase)) {
    addIssue('error', 'E001', '.planning/ directory not found', 'Run /wtdd-new-project to initialize');
    output({
      status: 'broken',
      errors,
      warnings,
  ...
  if (!fs.existsSync(projectPath)) {
    addIssue('error', 'E002', 'PROJECT.md not found', 'Run /wtdd-new-project to create');
  } else {
  ...
  if (!fs.existsSync(roadmapPath)) {
    addIssue('error', 'E003', 'ROADMAP.md not found', 'Run /wtdd-new-milestone to create roadmap');
  }

  // ─── Check 4: STATE.md exists and references valid phases ─────────────────
  if (!fs.existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run /wtdd-health --repair to regenerate', true);
      ...
      if (!diskPhases.has(ref) && !diskPhases.has(normalizedRef) && !diskPhases.has(String(parseInt(ref, 10)))) {
        ...
        if (diskPhases.size > 0) {
          addIssue(
            ...
            'Review STATE.md manually before changing it; /wtdd-health --repair will not overwrite an existing STATE.md for phase mismatches'
          );
        }
      }
    }
  ...
  if (!fs.existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run /wtdd-health --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${err.message}`, 'Run /wtdd-health --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && configParsed.workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run /wtdd-health --repair to add key', true);
    ...
    for (const e of phaseEntries) {
      ...
      if (hasResearch && !hasValidation) {
        ...
        if (researchContent.includes('## Validation Architecture')) {
          addIssue('warning', 'W009', `Phase ${e.name}: has Validation Architecture in RESEARCH.md but no VALIDATION.md`, 'Re-run /wtdd-plan-phase with --research to regenerate');
        }
      }
    }
  } catch { /* intentionally empty */ }
  ...
  // Verify WTDD agents are installed. Missing agents cause Task(subagent_type=...)
    ...
    if (!agentStatus.agents_installed) {
      if (agentStatus.installed_agents.length === 0) {
        addIssue('warning', 'W010',
          `No WTDD agents found in ${agentStatus.agents_dir} — Task(subagent_type="wtdd-*") will fall back to general-purpose`,
          'Run the WTDD installer: npx what-the-dog-doing-cc@latest');
      } else {
        addIssue('warning', 'W010',
          `Missing ${agentStatus.missing_agents.length} WTDD agents: ${agentStatus.missing_agents.join(', ')} — affected workflows will fall back to general-purpose`,
          'Run the WTDD installer: npx what-the-dog-doing-cc@latest');
      }
    }
        ...
        if (phaseCheckboxRe.test(roadmapContentFull)) {
          ...
          if (statusVal !== 'complete' && statusVal !== 'done') {
            addIssue('warning', 'W011',
              ...
              'Run /wtdd-progress to re-derive current position, or manually update STATE.md');
          }
        }
      }
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by /wtdd-health --repair\n`;
