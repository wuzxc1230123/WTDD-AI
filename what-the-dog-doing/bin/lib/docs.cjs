 ...
 * with WTDD marker detection, doc tooling detection, monorepo awareness, and
...
const WTDD_MARKER = '<!-- generated-by: wtdd-doc-writer -->';
...
/**
 * Check whether a file begins with the WTDD doc writer marker.
...
function hasGsdMarker(filePath) {
  try {
    ...
    fs.closeSync(fd);
    return buf.slice(0, bytesRead).toString('utf-8').includes(WTDD_MARKER);
  } catch {
    return false;
  }
}
 ...
 * @returns {Array<{path: string, has_wtdd_marker: boolean}>}
 */
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          results.push({ path: rel, has_wtdd_marker: hasGsdMarker(abs) });
        }
      }
 ...
 *
 * @example
 * node wtdd-tools.cjs docs-init --raw
 *
...
function cmdDocsInit(cwd, raw) {
  ...
  const result = {
    doc_writer_model: resolveModelInternal(cwd, 'wtdd-doc-writer'),
