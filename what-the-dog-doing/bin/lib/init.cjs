...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'wtdd-executor'),
    verifier_model: resolveModelInternal(cwd, 'wtdd-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'wtdd-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'wtdd-planner'),
    checker_model: resolveModelInternal(cwd, 'wtdd-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.wtdd', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // Detect Firecrawl API key availability
  const firecrawlKeyFile = path.join(homedir, '.wtdd', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.wtdd', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'wtdd-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'wtdd-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'wtdd-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'wtdd-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /wtdd-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /wtdd-new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/wtdd-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/wtdd-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/wtdd-discuss-phase ${phase.number}`,
      });
    }
  }
  ...
  const sanitizeFlags = (raw) => {
    ...
    if (!safe) {
      process.stderr.write(`wtdd-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
      return '';
    }
    return val;
  };
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'wtdd-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'wtdd-executor', 'wtdd-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills wtdd-executor 2>/dev/null)
 */
