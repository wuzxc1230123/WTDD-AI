...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /wtdd-new-project to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- WTDD:${sectionName}-start`;
  const endMarker = `<!-- WTDD:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- WTDD:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- WTDD:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateSkillsSection(cwd) {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip WTDD's own installed skills — only surface project-specific skills
      if (entry.name.startsWith('wtdd-')) continue;
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'what-the-dog-doing', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/wtdd-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'wtdd', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/wtdd-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by WTDD from ${dataSource}. Run \`/wtdd-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- WTDD:profile-start -->';
    const endMarker = '<!-- WTDD:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## WTDD Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- WTDD:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- WTDD:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- WTDD:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- WTDD:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /wtdd-profile-user to unlock Developer Profile.';
