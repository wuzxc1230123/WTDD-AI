...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
  ...
  reapStaleTempFiles('wtdd-pipeline-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
  ...
  reapStaleTempFiles('wtdd-profile-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-profile-'));
