...
```bash
PHASE_INFO=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" find-phase "${PHASE}" --raw)
```
