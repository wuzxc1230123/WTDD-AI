...
**Input:** Must-have artifact: `what-the-dog-doing/workflows/context-bridge.md: >50 lines`

**Output:**

> **L1 (Existence):** EXISTS -- `what-the-dog-doing/workflows/context-bridge.md` found, 187 lines.
...
> **L3 (Wired):** WIRED -- Referenced by `what-the-dog-doing/workflows/execute-phase.md` at step 3. Called by `agents/wtdd-executor.md` via @-reference include. Arguments verified: receives planning directory path matching the expected parameter.
> **Status: PASSED**
...
> **Method:** Ran `grep -n "scope" what-the-dog-doing/bin/validate-commit.js` to locate scope validation logic.
...
> **Ran:** `echo "feat(: test" | node what-the-dog-doing/bin/validate-commit.js` -- exit code 0 (accepted).
...
> **Method:** Ran `grep -n "feedback-loop\|extractFeedback" what-the-dog-doing/bin/check-subagent-output.js`.
...
> **Cross-check:** `grep -rn "require.*feedback-loop" what-the-dog-doing/bin/` returns zero results across all scripts.
...
> **Method:** Ran `grep "architecture_graph\|architecture_guard" what-the-dog-doing/bin/lib/config-schema.json`.
> **Evidence:** Zero matches in schema. However, `grep "architecture" what-the-dog-doing/bin/lib/config.cjs` shows both flags in CONFIG_DEFAULTS (lines 426, 428).
...
> **Method:** Ran `grep -rn "\/\[a-zA-Z\]\/" what-the-dog-doing/bin/` to find inline MSYS regex.
> **Evidence:** 2 matches in `commands/wtdd/misc.md` (lines 26, 487). These were NOT in the original plan scope -- Phase 148 extracted code into misc.md AFTER Phase 149's plan was written.
...
**Input:** Must-have artifact: `what-the-dog-doing/bin/lib/audit-checks/index.cjs: >100 lines`

**Output:**

> **L1 (Existence):** EXISTS -- File found at `what-the-dog-doing/bin/lib/audit-checks/index.cjs`, 142 lines.
> **Status: PASSED**
