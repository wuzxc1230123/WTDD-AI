...
The `--ws <name>` flag scopes WTDD operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `WTDD_WORKSTREAM` environment variable (per-instance)
...
WTDD now prefers a session-scoped pointer keyed by runtime/session identity
(`WTDD_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When WTDD resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `WTDD_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, WTDD does not keep probing. It falls
...
non-interactive, WTDD intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, WTDD also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, WTDD
...
WTDD does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${WTDD_WS}` which:
...
```bash
# All wtdd-tools commands accept --ws
node wtdd-tools.cjs state json --ws feature-a
node wtdd-tools.cjs find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
WTDD_SESSION_KEY=my-terminal-a node wtdd-tools.cjs workstream set feature-a
WTDD_SESSION_KEY=my-terminal-a node wtdd-tools.cjs state json
WTDD_SESSION_KEY=my-terminal-b node wtdd-tools.cjs workstream set feature-b
WTDD_SESSION_KEY=my-terminal-b node wtdd-tools.cjs state json

# Workstream CRUD
node wtdd-tools.cjs workstream create <name>
node wtdd-tools.cjs workstream list
node wtdd-tools.cjs workstream status <name>
node wtdd-tools.cjs workstream complete <name>
```
