...
10. **NEVER** use non-WTDD agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "wtdd-{agent}"` (e.g., `wtdd-phase-researcher`, `wtdd-executor`, `wtdd-planner`). WTDD agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `wtdd-tools.cjs` CLI commands (`state update`, `state advance-plan`, `roadmap update-status`) for mutations. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- WTDD uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Always use `wtdd-tools.cjs`** (not `wtdd-tools.js` or any other variant) -- WTDD uses CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- wtdd-tools detection depends on this exact pattern.
