...
| `git.phase_branch_template` | `"wtdd/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"wtdd/{milestone}-{slug}"` | Branch template for milestone strategy |
...
| `manager.flags.discuss` | `""` | Flags passed to `/wtdd-discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using wtdd-tools.cjs (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: update state" --files .planning/STATE.md
...
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
**Note:** Most WTDD operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Standard WTDD behavior

**When `git.branching_strategy: "phase"`:**
- `execute-phase` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `wtdd/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `wtdd/v1.0-mvp`)
...
| `git.phase_branch_template` | string | `"wtdd/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"wtdd/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/wtdd-discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.wtdd/<name>_api_key` file). Setting them to `true` without the API key has no effect.
...
8. **`sub_repos` auto-sync** -- On every config load, WTDD scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. Legacy `multiRepo: true` is automatically migrated to a detected `sub_repos` array.
