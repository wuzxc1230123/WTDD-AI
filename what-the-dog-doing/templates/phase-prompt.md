...
> **Note:** Planning methodology is in `agents/wtdd-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/what-the-dog-doing/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/wtdd-plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/what-the-dog-doing/references/tdd.md` for TDD plan structure.
...
See `~/.claude/what-the-dog-doing/templates/user-setup.md` for full schema and examples
...
See `~/.claude/what-the-dog-doing/workflows/verify-phase.md` for verification logic.
