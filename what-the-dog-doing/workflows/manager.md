...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init manager)
...
- `manager_flags.discuss` — appended to `/wtdd-discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` — appended to plan agent init command
- `manager_flags.execute` — appended to execute agent init command

These are empty strings by default. Set via: `wtdd-tools config-set manager.flags.discuss "--auto --analyze"`
...
All {phase_count} phases done. Ready for final steps:
  → /wtdd-verify-work — run acceptance testing
  → /wtdd-complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="wtdd-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="wtdd-complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in WTDD config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="wtdd-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Task(
  ...
  prompt="You are running the WTDD plan-phase workflow for phase {N} of the project.
...
Skill(skill=\"wtdd-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the WTDD execute-phase workflow for phase {N} of the project.
...
Skill(skill=\"wtdd-execute-phase\", args=\"{N} {manager_flags.execute}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="wtdd-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="wtdd-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="wtdd-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="wtdd-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /wtdd-manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/wtdd-manager` or `/wtdd-progress` invocation.
