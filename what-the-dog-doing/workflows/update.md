<purpose>
Check for WTDD updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether WTDD is installed locally or globally by checking both locations and validating install integrity.

First, derive `PREFERRED_CONFIG_DIR` and `PREFERRED_RUNTIME` from the invoking prompt's `execution_context` path:
- If the path contains `/what-the-dog-doing/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/wtdd-update` targets the runtime that invoked it.
...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/what-the-dog-doing/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/what-the-dog-doing/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/what-the-dog-doing/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/what-the-dog-doing/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/what-the-dog-doing/VERSION")"
  else
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/what-the-dog-doing/VERSION" ] || [ -f "./$dir/what-the-dog-doing/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/what-the-dog-doing/VERSION"
    LOCAL_MARKER_FILE="./$dir/what-the-dog-doing/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/what-the-dog-doing/VERSION" ] || [ -f "$dir/what-the-dog-doing/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/what-the-dog-doing/VERSION"
    GLOBAL_MARKER_FILE="$dir/what-the-dog-doing/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/what-the-dog-doing/VERSION" ] || [ -f "$HOME/$dir/what-the-dog-doing/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/what-the-dog-doing/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/what-the-dog-doing/workflows/update.md"
...
```bash
npm view what-the-dog-doing-cc version 2>/dev/null
```
...
To update manually: `npx what-the-dog-doing-cc --global`
```
...
⚠️  **Note:** The installer performs a clean install of WTDD folders:
- `commands/wtdd/` will be wiped and replaced
- `what-the-dog-doing/` will be wiped and replaced
- `agents/wtdd-*` files will be replaced
...
- Custom commands not in `commands/wtdd/` ✓
- Custom agents not prefixed with `wtdd-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any WTDD files directly, they'll be automatically backed up to `wtdd-local-patches/` and can be reapplied with `/wtdd-reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y what-the-dog-doing-cc@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y what-the-dog-doing-cc@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y what-the-dog-doing-cc@latest --claude --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/wtdd-update-check.json"
  fi
done

for dir in .claude .config/opencode .opencode .gemini .config/kilo .kilo .codex; do
  rm -f "./$dir/cache/wtdd-update-check.json"
  rm -f "$HOME/$dir/cache/wtdd-update-check.json"
done
```

The SessionStart hook (`wtdd-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  WTDD Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/wtdd-build/what-the-dog-doing/blob/main/CHANGELOG.md)
```
</step>
...
Check for wtdd-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /wtdd-reapply-patches to merge your modifications into the new version.
```
