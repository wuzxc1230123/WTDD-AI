...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing wtdd-tools.cjs subcommands and the wtdd-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/what-the-dog-doing/references/ui-brand.md (display patterns)
- @$HOME/.claude/what-the-dog-doing/agents/wtdd-user-profiler.md (profiler agent definition)
- @$HOME/.claude/what-the-dog-doing/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
cp "$HOME/.claude/what-the-dog-doing/USER-PROFILE.md" "$HOME/.claude/what-the-dog-doing/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at $HOME/.claude/what-the-dog-doing/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /wtdd-profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn wtdd-user-profiler agent using Task tool:**

Use the Task tool to spawn the `wtdd-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/what-the-dog-doing/references/user-profiling.md`
...
Reference: @$HOME/.claude/what-the-dog-doing/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/wtdd-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/wtdd-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/what-the-dog-doing/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/wtdd-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/what-the-dog-doing/USER-PROFILE.md" and jump to step 10.
...
**For /wtdd-dev-preferences (if selected):**

```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /wtdd-dev-preferences at $HOME/.claude/commands/wtdd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any wtdd-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/what-the-dog-doing/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/what-the-dog-doing/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /wtdd-dev-preferences   $HOME/.claude/commands/wtdd/dev-preferences.md
...
- [ ] Artifacts generated sequentially via wtdd-tools.cjs subcommands
