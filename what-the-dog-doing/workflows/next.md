<purpose>
Detect current project state and automatically advance to the next logical WTDD workflow step.
...
```bash
# Get state snapshot
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```
No WTDD project detected. Run `/wtdd-new-project` to get started.
```
Exit.
</step>
...
Run `/wtdd-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
- If file exists and count >= 6: prompt "You've called /wtdd-next {N} times consecutively. Continue? [y/N]"
- If user says no, exit
- Increment the counter
- The counter file is deleted by any non-`/wtdd-next` command (convention — other workflows don't need to implement this, the note here is sufficient)
</step>
...
→ Next action: `/wtdd-discuss-phase <first-phase>`
...
▶ **Next step:** `/wtdd-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/wtdd-next` is zero-friction advancement.
</step>
