...
<required_reading>
@~/.claude/what-the-dog-doing/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init resume)
...
**If `planning_exists` is false:** This is a new project - route to /wtdd-new-project
</step>
...
- This is the primary resumption source — structured data from `/wtdd-pause-work`
...
📋 [N] pending todos — /wtdd-check-todos to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/wtdd-execute-phase {phase} ${WTDD_WS})
   OR
1. Discuss Phase 3 context (/wtdd-discuss-phase 3 ${WTDD_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/wtdd-plan-phase 3 ${WTDD_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show command for user to run after clearing:
  ...
  **Also available:**
  - `/wtdd-discuss-phase [N] ${WTDD_WS}` — gather context first
  - `/wtdd-research-phase [N] ${WTDD_WS}` — investigate unknowns
