<purpose>
Remove a WTDD workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/wtdd-list-workspaces` to show available workspaces, then ask:
