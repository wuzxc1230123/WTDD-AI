<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /wtdd-plan-phase --gaps.
...
<template>
@~/.claude/what-the-dog-doing/templates/UAT.md
</template>
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init verify-work "${PHASE_ARG}")
...
Provide a phase number to start testing (e.g., /wtdd-verify-work 4)
```
...
```
UI_PHASE_FLAG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
⚠ Security enforcement enabled — /wtdd-secure-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/wtdd-secure-phase {phase}` — security review (required before advancing)
- `/wtdd-plan-phase {next}` — Plan next phase
- `/wtdd-execute-phase {next}` — Execute next phase
- `/wtdd-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
- `/wtdd-plan-phase {next}` — Plan next phase
- `/wtdd-execute-phase {next}` — Execute next phase
- `/wtdd-secure-phase {phase}` — security review
- `/wtdd-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
- Follow @~/.claude/what-the-dog-doing/workflows/diagnose-issues.md
...
Spawn wtdd-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /wtdd-execute-phase
...
Spawn wtdd-planner with revision context:
...
3. Abandon (exit, user runs /wtdd-plan-phase manually)
...
`/clear` then `/wtdd-execute-phase {phase} --gaps-only`
...
- [ ] If issues: wtdd-planner creates fix plans (gap_closure mode)
- [ ] If issues: wtdd-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/wtdd-execute-phase --gaps-only` when complete
</success_criteria>
