...
```bash
CONFIG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /wtdd-complete-milestone (if last phase in milestone)
- /wtdd-progress (to see what's next)
...
- /wtdd-complete-milestone — if all phases in milestone are done
- /wtdd-progress — see overall project state
- /wtdd-execute-phase {next} — continue to next phase
</offer_next>
