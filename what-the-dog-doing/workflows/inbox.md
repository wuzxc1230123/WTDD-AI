...
```bash
gh issue close {number} --comment "Closed by WTDD inbox triage: this issue is missing required fields per the issue template. Missing: {list}. Please reopen with a complete submission. See CONTRIBUTING.md for requirements."
```

For PRs with gate violations:
```bash
gh pr close {number} --comment "Closed by WTDD inbox triage: this PR does not meet the issue-first requirement. {specific violation}. See CONTRIBUTING.md for the correct process."
```
...
- /wtdd-review — Run cross-AI peer review on a specific phase plan
- /wtdd-ship — Create a PR from completed work
- /wtdd-progress — See overall project state
- /wtdd-inbox --label — Re-run with auto-labeling enabled
</offer_next>
