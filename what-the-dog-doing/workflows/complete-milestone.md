...
2. **Run audit first** — `/wtdd-audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `wtdd-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to wtdd-tools:**

```bash
ARCHIVE=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/wtdd-cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `~/.claude/what-the-dog-doing/templates/retrospective.md`.
...
**Commit:**
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
Detect base branch:
```bash
BASE_BRANCH=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/wtdd-new-milestone)
