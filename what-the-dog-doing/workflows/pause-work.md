<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/wtdd-resume-work`; the markdown provides human-readable context.
</purpose>
...
```bash
timestamp=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
