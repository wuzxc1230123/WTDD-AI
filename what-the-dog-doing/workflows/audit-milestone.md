...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" resolve-model wtdd-integration-checker --raw)
```
...
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="wtdd-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/wtdd-validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/wtdd-validate-phase {N}` |

Phases needing validation: run `/wtdd-validate-phase {N}` for each flagged phase.
...
- /wtdd-complete-milestone {version} — proceed anyway (accept tech debt)
