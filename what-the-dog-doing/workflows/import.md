...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via wtdd-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  WTDD > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /wtdd-import --from <path>

  --from <path>   Import an external plan file into WTDD format
```
...
2. Read `.planning/PROJECT.md` — extract project constraints, tech stack, scope boundaries.
   **If PROJECT.md does not exist:** skip constraint checks that rely on it and display:
   ```
   WTDD > Note: No PROJECT.md found. Conflict checks against project constraints will be skipped.
   ```
...
- **WTDD PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Display:
```
WTDD > BLOCKED: {N} blockers must be resolved before import can proceed.
```
...
Convert the imported content to WTDD PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to WTDD `{NN}-{MM}-PLAN.md` convention during conversion.

Apply WTDD naming convention for the output filename:
...
Delegate validation to wtdd-plan-checker:

```
Task({
  subagent_type: "wtdd-plan-checker",
  prompt: "Validate: .planning/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and WTDD conventions. Report any issues."
})
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```

Display completion:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► IMPORT COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `wtdd-plan-checker` and `wtdd-planner`
- Write `.planning/.active-skill` — this is a PBR pattern with no WTDD equivalent
