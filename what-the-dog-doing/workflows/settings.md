<purpose>
Interactive configuration of WTDD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.wtdd/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-ensure-section
...
- `workflow.ui_safety_gate` — prompt to run /wtdd-ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /wtdd-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /wtdd-ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (wtdd/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (wtdd/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /wtdd-autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.wtdd/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.wtdd/defaults.json`:

```bash
mkdir -p ~/.wtdd
```

Write `~/.wtdd/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /wtdd-plan-phase and /wtdd-execute-phase runs.

Quick commands:
- /wtdd-set-profile <profile> — switch model profile
- /wtdd-plan-phase --research — force research
- /wtdd-plan-phase --skip-research — skip research
- /wtdd-plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.wtdd/defaults.json)
