...
<required_reading>
@~/.claude/what-the-dog-doing/references/domain-probes.md
@~/.claude/what-the-dog-doing/references/gate-prompts.md
@~/.claude/what-the-dog-doing/references/universal-anti-patterns.md
</required_reading>

<downstream_awareness>
**CONTEXT.md feeds into:**

1. **wtdd-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **wtdd-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/wtdd-discuss-phase --text`)
- Per-project: `wtdd-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/wtdd-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-advisor 2>/dev/null)
```
...
Use /wtdd-progress ${WTDD_WS} to see available phases.
```
Exit workflow.
...
- Read and execute @~/.claude/what-the-dog-doing/workflows/discuss-phase-power.md end-to-end
...
- options:
  - "Continue and replan after" — Capture context, then run /wtdd-plan-phase {X} ${WTDD_WS} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.claude/what-the-dog-doing/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" resolve-model wtdd-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/wtdd-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
This is a structured checkpoint, not the final CONTEXT.md — the `write_context` step still produces the canonical output. But if the session dies, the next `/wtdd-discuss-phase` invocation can detect this checkpoint and offer to resume from it instead of starting from scratch.
...
`/wtdd-plan-phase ${PHASE} ${WTDD_WS}`

---

**Also available:**
- `/wtdd-discuss-phase ${PHASE} --chain ${WTDD_WS}` — re-run with auto plan+execute after
- `/wtdd-plan-phase ${PHASE} --skip-research ${WTDD_WS}` — plan without research
- `/wtdd-ui-phase ${PHASE} ${WTDD_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="wtdd-plan-phase", args="${PHASE} --auto ${WTDD_WS}")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: /wtdd-discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${WTDD_WS}
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /wtdd-execute-phase ${PHASE} ${WTDD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /wtdd-plan-phase ${PHASE} ${WTDD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /wtdd-plan-phase ${PHASE} --gaps ${WTDD_WS}
  ```
...
**Full step-by-step instructions:** @~/.claude/what-the-dog-doing/workflows/discuss-phase-power.md
