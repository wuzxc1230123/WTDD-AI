<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns wtdd-code-reviewer agent, commits REVIEW.md, and presents results to user.
</purpose>
...
- wtdd-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
```
Error: Phase ${PHASE_ARG} not found. Run /wtdd-status to see available phases.
```
...
2. Config value: `node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /wtdd-code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
Spawn the wtdd-code-reviewer agent:

```
Task(subagent_type="wtdd-code-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
No REVIEW.md created. You can retry with /wtdd-code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  if [ "$HAS_STATUS" = "valid" ]; then
    ...
    if [ "$COMMIT_DOCS" = "true" ]; then
      node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit \
  ...
  echo "No REVIEW.md to commit. Please retry with /wtdd-code-review ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /wtdd-code-review-fix ${PHASE_NUMBER}  — Auto-fix issues
