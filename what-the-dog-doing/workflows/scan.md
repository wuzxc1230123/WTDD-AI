<purpose>
Lightweight codebase assessment. Spawns a single wtdd-codebase-mapper agent for one focus area,
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `wtdd-codebase-mapper` agent with the selected focus area:
...
Use `/wtdd-map-codebase` for a comprehensive 4-area parallel scan.
```
