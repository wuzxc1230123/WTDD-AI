...
- **Claude Code:** Uses `Task(subagent_type="wtdd-executor", ...)` — blocks until complete, returns result
...
@~/.claude/what-the-dog-doing/references/agent-contracts.md
@~/.claude/what-the-dog-doing/references/context-budget.md
@~/.claude/what-the-dog-doing/references/gates.md
</required_reading>

<available_agent_types>
These are the valid WTDD subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- wtdd-executor — Executes plan tasks, commits, creates SUMMARY.md
- wtdd-verifier — Verifies phase completion, checks quality gates
- wtdd-planner — Creates detailed plans from phase scope
- wtdd-phase-researcher — Researches technical approaches for a phase
- wtdd-plan-checker — Reviews plan quality before execution
- wtdd-debugger — Diagnoses and fixes issues
- wtdd-codebase-mapper — Maps project structure and dependencies
- wtdd-integration-checker — Checks cross-phase integration
- wtdd-nyquist-auditor — Validates verification coverage
- wtdd-ui-researcher — Researches UI/UX approaches
- wtdd-ui-checker — Reviews UI implementation quality
- wtdd-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
Check if the current runtime is Copilot by testing for the `@wtdd-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/what-the-dog-doing/workflows/execute-plan.md` **inline**
...
- Maintains WTDD's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning WTDD
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For wtdd-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @~/.claude/what-the-dog-doing/workflows/execute-plan.md
       @~/.claude/what-the-dog-doing/templates/summary.md
       @~/.claude/what-the-dog-doing/references/checkpoints.md
       @~/.claude/what-the-dog-doing/references/tdd.md
       </execution_context>
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get commit_docs 2>/dev/null || echo "true")
...
5.6. **Post-wave shared artifact update (worktree mode only):**
   ...
   for PLAN_ID in ${WAVE_PLAN_IDS}; do
     node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${PLAN_ID}" completed
   done
...
7. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a WTDD or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 5 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**Security gate check:**
```bash
SECURITY_CFG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
/wtdd-execute-phase {phase} ${WTDD_WS}                # Continue remaining waves
/wtdd-execute-phase {phase} --wave {next} ${WTDD_WS}  # Run the next wave explicitly
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
**Invoke review:**
```
Skill(skill="wtdd:code-review", args="${PHASE_NUMBER}")
```
...
/wtdd-code-review-fix ${PHASE_NUMBER}
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${WTDD_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but WTDD_SKIP_SCHEMA_CHECK=true — bypassing gate.
...
2. Skip schema check (WTDD_SKIP_SCHEMA_CHECK=true) — bypass this gate
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-verifier 2>/dev/null)
```
...
| `gaps_found` | Present gap summary, offer `/wtdd-plan-phase {phase} --gaps ${WTDD_WS}` |
...
Commit the file:
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/wtdd-progress` and `/wtdd-audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/wtdd-verify-work` on it.
...
`/wtdd-plan-phase {X} --gaps ${WTDD_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/wtdd-verify-work {X} ${WTDD_WS}` — manual testing first
```

Gap closure cycle: `/wtdd-plan-phase {X} --gaps ${WTDD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/wtdd-execute-phase {X} --gaps-only ${WTDD_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/wtdd-progress` and `/wtdd-audit-uat`.
```

```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
entries from the completed phase to the global learnings store at `~/.wtdd/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" learnings copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/wtdd-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/what-the-dog-doing/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/wtdd-transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/wtdd-progress ${WTDD_WS} — see updated roadmap
/wtdd-discuss-phase {next} ${WTDD_WS} — discuss next phase before planning
/wtdd-plan-phase {next} ${WTDD_WS} — plan next phase
/wtdd-execute-phase {next} ${WTDD_WS} — execute next phase
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not WTDD. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/wtdd-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
