...
<required_reading>
@~/.claude/what-the-dog-doing/references/verification-patterns.md
@~/.claude/what-the-dog-doing/templates/verification-report.md
</required_reading>
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap analyze
```
...
Use wtdd-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use wtdd-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify artifacts "$plan")
...
Use wtdd-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify key-links "$plan")
...
See ~/.claude/what-the-dog-doing/templates/verification-report.md for complete template.
</step>
