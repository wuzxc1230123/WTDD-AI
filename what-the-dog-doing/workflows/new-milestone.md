...
- Check for MILESTONE-CONTEXT.md (from /wtdd-discuss-milestone)
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phases clear --confirm
```

```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/wtdd-new-milestone --reset-phase-numbers ${WTDD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/wtdd-plan-phase` behavior. To change the default, use `/wtdd-settings`.
...
Spawn 4 parallel wtdd-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/what-the-dog-doing/templates/research-project/{FILE}
</output>
", subagent_type="wtdd-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/wtdd-discuss-phase [N] ${WTDD_WS}` — gather context and clarify approach

Also: `/wtdd-plan-phase [N] ${WTDD_WS}` — skip discussion, plan directly
```
...
- [ ] wtdd-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] Phase numbering mode respected (continued or reset)
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/wtdd-discuss-phase [N] ${WTDD_WS}`
