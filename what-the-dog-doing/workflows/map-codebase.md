...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-codebase-mapper 2>/dev/null)
```
...
Spawn 4 parallel wtdd-codebase-mapper agents.

Use Task tool with `subagent_type="wtdd-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `wtdd-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="wtdd-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `wtdd-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/wtdd-map-codebase`
...
- If Task tool available: 4 parallel wtdd-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in WTDD style
</success_criteria>
