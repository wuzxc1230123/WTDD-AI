...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /wtdd-new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /wtdd-new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /wtdd-new-project    # Initialize WTDD in the workspace
```
...
**Offer to initialize WTDD (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize WTDD"
- question: "Would you like to initialize a WTDD project in the new workspace?"
- options:
  - "Yes — run /wtdd-new-project" → tell user to `cd $TARGET_PATH` first, then run `/wtdd-new-project`
