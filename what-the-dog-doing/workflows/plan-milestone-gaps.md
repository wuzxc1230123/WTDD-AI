<purpose>
Create all phases necessary to close gaps identified by `/wtdd-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/wtdd-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/wtdd-audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/wtdd-execute-phase {N}` — if plans already exist
...
`/wtdd-audit-milestone` — re-audit to verify gaps closed
`/wtdd-complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/wtdd-plan-phase` next
</success_criteria>
