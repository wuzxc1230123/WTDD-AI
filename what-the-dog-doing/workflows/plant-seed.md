...
Seeds auto-surface during /wtdd-new-milestone when trigger conditions match the
...
This seed should be presented during `/wtdd-new-milestone` when the milestone
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /wtdd-new-milestone
