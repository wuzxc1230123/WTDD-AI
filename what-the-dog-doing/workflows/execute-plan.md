...
@~/.claude/what-the-dog-doing/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via wtdd-tools config-get if needed
```
...
2. Per segment:
   - Subagent route: spawn wtdd-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/what-the-dog-doing/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/what-the-dog-doing/bin/wtdd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/what-the-dog-doing/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/what-the-dog-doing/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/what-the-dog-doing/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/what-the-dog-doing/templates/summary.md`.
...
Update STATE.md using wtdd-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state record-metric \
...
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using wtdd-tools:
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/wtdd-execute-phase {phase}` + `/wtdd-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/wtdd-plan-phase {Z+1}` + `/wtdd-verify-work {Z}` + `/wtdd-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/wtdd-complete-milestone` + `/wtdd-verify-work` + `/wtdd-add-phase` |
