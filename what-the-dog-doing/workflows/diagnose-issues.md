...
```bash
AGENT_SKILLS_DEBUGGER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-debugger 2>/dev/null)
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
