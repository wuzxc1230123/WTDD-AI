...
offers mid-conversation research when useful, then routes crystallized outputs to WTDD artifacts.
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@~/.claude/what-the-dog-doing/references/questioning.md
...
- wtdd-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
- **Phases:** Use existing `/wtdd-add-phase` command via SlashCommand

Commit if `commit_docs` is enabled:
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/wtdd-explore` or start working with `/wtdd-next`.
```
