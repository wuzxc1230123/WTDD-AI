<purpose>
Safe git revert workflow. Rolls back WTDD phase or plan commits using the phase manifest with dependency checks and a confirmation gate. Uses git revert --no-commit (NEVER git reset) to preserve history.
</purpose>
...
```
Usage: /wtdd-undo --last N | --phase NN | --plan NN-MM

Modes:
  --last N      Show last N WTDD commits for interactive selection
...
Filter for WTDD conventional commits matching `type(scope): message` pattern (e.g., `feat(04-01):`, `docs(03):`, `fix(02-03):`).
...
```
Working tree has uncommitted changes. Commit or stash them before running /wtdd-undo.
```
...
**Also available:**
- `/wtdd-execute-phase ${PHASE}` — re-execute if needed
- `/wtdd-undo --last 1` — undo the revert itself if something went wrong
