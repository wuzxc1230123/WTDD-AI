<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates wtdd-phase-researcher, wtdd-planner, and wtdd-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@~/.claude/what-the-dog-doing/references/revision-loop.md
...
Valid WTDD subagent types (use exact names — do not fall back to 'general-purpose'):
...
```bash
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-checker 2>/dev/null)
...
**If `planning_exists` is false:** Error — run `/wtdd-new-project` first.
...
Then re-run /wtdd-plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /wtdd-plan-phase {X} ${WTDD_WS}:

  /wtdd-discuss-phase {X} ${WTDD_WS}
  ```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn wtdd-phase-researcher

```bash
PHASE_DESC=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /wtdd-discuss-phase)
...
Task(
  ...
  subagent_type="wtdd-phase-researcher",
...
**If found:**
1. Read template: `~/.claude/what-the-dog-doing/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
UI_PHASE_CFG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```
Skill(skill="wtdd-ui-phase", args="${PHASE} --auto ${WTDD_WS}")
```
After `wtdd-ui-phase` returns, re-read:
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
...
1. Generate UI-SPEC first — Run /wtdd-ui-phase {N} then re-run /wtdd-plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" → Display: "Run `/wtdd-ui-phase {N} ${WTDD_WS}` then re-run `/wtdd-plan-phase {N} ${WTDD_WS}`". Exit workflow.
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
```bash
_wtdd_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_wtdd_field "$INIT" state_path)
ROADMAP_PATH=$(_wtdd_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_wtdd_field "$INIT" requirements_path)
RESEARCH_PATH=$(_wtdd_field "$INIT" research_path)
VERIFICATION_PATH=$(_wtdd_field "$INIT" verification_path)
UAT_PATH=$(_wtdd_field "$INIT" uat_path)
CONTEXT_PATH=$(_wtdd_field "$INIT" context_path)
REVIEWS_PATH=$(_wtdd_field "$INIT" reviews_path)
```
...
1. Re-run: `/wtdd-plan-phase {PHASE} --research ${WTDD_WS}`
2. Disable Nyquist with the exact command:
   `node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn wtdd-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<downstream_consumer>
Output consumed by /wtdd-execute-phase. Plans need:
...
**If "Split":** Use `/wtdd-insert-phase` to create the sub-phases, then replan each.
...
## 10. Spawn wtdd-plan-checker Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► VERIFYING PLANS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Task(
  ...
  subagent_type="wtdd-plan-checker",
  model="{checker_model}",
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/wtdd-debug` to investigate root causes."
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-set workflow._auto_chain_active true
fi
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► AUTO-ADVANCING TO EXECUTE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
Skill(skill="wtdd-execute-phase", args="${PHASE} --auto --no-transition ${WTDD_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   WTDD ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /wtdd-discuss-phase ${NEXT_PHASE} --auto ${WTDD_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /wtdd-execute-phase ${PHASE} ${WTDD_WS}
  ```
...
/wtdd-execute-phase {X} ${WTDD_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /wtdd-plan-phase {X} --research — re-research first
- /wtdd-review --phase {X} --all — peer review plans with external AIs
- /wtdd-plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/wtdd-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/wtdd-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] wtdd-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] wtdd-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] wtdd-plan-checker spawned with CONTEXT.md
