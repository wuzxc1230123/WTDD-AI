...
- wtdd-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" resolve-model wtdd-security-auditor --raw)
...
If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /wtdd-settings."

Display banner: `WTDD > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /wtdd-execute-phase {N} first."
...
## 5. Spawn wtdd-security-auditor

```
Task(
  prompt="Read ~/.claude/agents/wtdd-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="wtdd-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/what-the-dog-doing/templates/SECURITY.md`
...
```
WTDD > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /wtdd-secure-phase {N}
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
WTDD > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /wtdd-validate-phase {N}    validate test coverage
▶ /wtdd-verify-work {N}       run UAT
```
