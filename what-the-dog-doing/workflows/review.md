...
Write to a temp file: `/tmp/wtdd-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/wtdd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/wtdd-review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/wtdd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/wtdd-review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/wtdd-review-prompt-{phase}.md)" 2>/dev/null > /tmp/wtdd-review-codex-{phase}.md
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/wtdd-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
cat /tmp/wtdd-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/wtdd-review-opencode-{phase}.md
if [ ! -s /tmp/wtdd-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/wtdd-review-opencode-{phase}.md
fi
```
...
Commit:
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/wtdd-plan-phase --reviews)
</success_criteria>
