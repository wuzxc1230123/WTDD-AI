...
There is no `/wtdd-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/wtdd-transition`.

**Valid user commands for phase progression:**
- `/wtdd-discuss-phase {N}` — discuss a phase before planning
- `/wtdd-plan-phase {N}` — plan a phase
- `/wtdd-execute-phase {N}` — execute a phase
- `/wtdd-progress` — see roadmap progress
...
These will carry forward as debt. Review: `/wtdd-audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to wtdd-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `wtdd-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `wtdd-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `WTDD_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$WTDD_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" workstream list --raw)
fi
```
...
Filter out the current workstream (`$WTDD_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/wtdd-plan-phase [X+1] --auto ${WTDD_WS}")
...
Exit skill and invoke SlashCommand("/wtdd-discuss-phase [X+1] --auto ${WTDD_WS}")
...
`/wtdd-discuss-phase [X+1] ${WTDD_WS}` — gather context and clarify approach

---

**Also available:**
- `/wtdd-plan-phase [X+1] ${WTDD_WS}` — skip discussion, plan directly
- `/wtdd-research-phase [X+1] ${WTDD_WS}` — investigate unknowns
...
`/wtdd-plan-phase [X+1] ${WTDD_WS}`

---

**Also available:**
- `/wtdd-discuss-phase [X+1] ${WTDD_WS}` — revisit context
- `/wtdd-research-phase [X+1] ${WTDD_WS}` — investigate unknowns
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/wtdd-workstreams complete {current_ws_name} ${WTDD_WS}`

See overall milestone progress:

`/wtdd-workstreams progress ${WTDD_WS}`
...
Do NOT suggest `/wtdd-complete-milestone` or `/wtdd-new-milestone`.
...
Exit skill and invoke SlashCommand("/wtdd-complete-milestone {version} ${WTDD_WS}")
...
`/wtdd-complete-milestone {version} ${WTDD_WS}`
