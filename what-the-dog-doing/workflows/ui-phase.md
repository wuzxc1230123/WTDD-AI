<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates wtdd-ui-researcher and wtdd-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" agent-skills wtdd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" resolve-model wtdd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" resolve-model wtdd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /wtdd-settings.
```
Exit workflow.
...
Recommended: run /wtdd-discuss-phase {N} first to capture design preferences.
...
## 5. Spawn wtdd-ui-researcher

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► UI DESIGN CONTRACT — PHASE {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/wtdd-ui-researcher.md for instructions.
...
Template: ~/.claude/what-the-dog-doing/templates/UI-SPEC.md
</output>
...
Task(
  ...
  subagent_type="wtdd-ui-researcher",
...
## 7. Spawn wtdd-ui-checker

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WTDD ► VERIFYING UI-SPEC
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/wtdd-ui-checker.md for instructions.
...
- Re-spawn wtdd-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /wtdd-ui-phase
...
```bash
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] wtdd-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] wtdd-ui-checker spawned with UI-SPEC.md
