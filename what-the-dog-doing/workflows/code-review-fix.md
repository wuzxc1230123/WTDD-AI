<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns wtdd-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- wtdd-code-fixer: Applies fixes to code review findings
- wtdd-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /wtdd-code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the wtdd-code-fixer agent with config:
...
```
Task(subagent_type="wtdd-code-fixer", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /wtdd-code-review-fix ${PHASE_ARG}.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn wtdd-code-reviewer agent to re-review
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /wtdd-code-review-fix ${PHASE_ARG}"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /wtdd-verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /wtdd-code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /wtdd-verify-work                           — Verify phase completion"
  echo ""
fi
```
