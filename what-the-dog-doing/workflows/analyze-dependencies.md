<purpose>
Analyze ROADMAP.md phases for dependency relationships before execution. Detect file overlap between phases, semantic API/data-flow dependencies, and suggest `Depends on` entries to prevent merge conflicts during parallel execution by `/wtdd-manager`.
</purpose>
...
Read `.planning/ROADMAP.md`. If it does not exist, error: "No ROADMAP.md found — run `/wtdd-new-project` first."
...
After applying: "ROADMAP.md updated. Run `/wtdd-manager` to execute phases in the correct order."
