...
const TOOLS_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'bin', 'wtdd-tools.cjs');
const TEST_ENV_BASE = {
  WTDD_SESSION_KEY: '',
  CODEX_THREAD_ID: '',
  CLAUDE_SESSION_ID: '',
...
function createTempDir(prefix = 'wtdd-test-') {
  return fs.mkdtempSync(path.join(require('os').tmpdir(), prefix));
}

// Create temp directory structure
function createTempProject(prefix = 'wtdd-test-') {
...
function createTempGitProject(prefix = 'wtdd-test-') {
  const tmpDir = fs.mkdtempSync(path.join(require('os').tmpdir(), prefix));
  fs.mkdirSync(path.join(tmpDir, '.planning', 'phases'), { recursive: true });
