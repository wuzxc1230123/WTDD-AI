/**
 * WTDD Secure-Phase Tests
 *
 * Validates the security-first enforcement layer:
 * - wtdd-security-auditor agent frontmatter and structure
...
const AGENTS_DIR = path.join(REPO_ROOT, 'agents', 'wtdd');
const COMMANDS_DIR = path.join(REPO_ROOT, 'commands', 'wtdd');
const WORKFLOWS_DIR = path.join(REPO_ROOT, 'what-the-dog-doing', 'workflows');
const TEMPLATES_DIR = path.join(REPO_ROOT, 'what-the-dog-doing', 'templates');

// ─── 1. Agent frontmatter — wtdd-security-auditor.md ─────────────────────────

describe('SECURE: wtdd-security-auditor agent', () => {
  const agentPath = path.join(AGENTS_DIR, 'wtdd-security-auditor.md');

  test('agent file exists', () => {
    assert.ok(
      fs.existsSync(agentPath),
      'wtdd-security-auditor.md must exist in agents/'
    );
  });
  ...
  test('name is wtdd-security-auditor', () => {
    const content = fs.readFileSync(agentPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: wtdd-security-auditor'),
      'name must be wtdd-security-auditor'
    );
  });
...
describe('SECURE: secure-phase command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in commands/wtdd/'
    );
  });

  test('has valid frontmatter with name wtdd:secure-phase', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: wtdd:secure-phase'),
      'name must be wtdd:secure-phase'
    );
  });
...
describe('SECURE: secure-phase workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in what-the-dog-doing/workflows/'
    );
  });

  test('contains wtdd-security-auditor reference', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
      content.includes('wtdd-security-auditor'),
      'must reference wtdd-security-auditor agent'
    );
  });
...
describe('SECURE: SECURITY.md template', () => {
  ...
  test('template exists', () => {
    assert.ok(
      ...
      'SECURITY.md must exist in what-the-dog-doing/templates/'
    );
  });
...
describe('SECURE: config.json security defaults', () => {
  ...
  test('config template exists', () => {
    assert.ok(
      ...
      'config.json must exist in what-the-dog-doing/templates/'
    );
  });
...
describe('SECURE: VALIDATION.md security columns', () => {
  ...
  test('VALIDATION.md template exists', () => {
    assert.ok(
      ...
      'VALIDATION.md must exist in what-the-dog-doing/templates/'
    );
  });
