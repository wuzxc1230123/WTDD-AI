/**
 * WTDD Tools Tests - autonomous --interactive flag
 *
...
describe('autonomous --interactive flag (#1413)', () => {
  const workflowPath = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'autonomous.md');
  const commandPath = path.join(__dirname, '..', 'commands', 'wtdd', 'autonomous.md');
  ...
  test('workflow uses discuss-phase skill in interactive mode', () => {
    const content = fs.readFileSync(workflowPath, 'utf8');
    assert.ok(
      content.includes('wtdd:discuss-phase') && content.includes('INTERACTIVE'),
      'workflow should invoke wtdd:discuss-phase when INTERACTIVE is set'
    );
  });
