...
const { resolveModelInternal } = require('../what-the-dog-doing/bin/lib/core.cjs');
...
describe('bug #1829: model_profile "inherit" — resolveModelInternal', () => {
  ...
  test('returns "inherit" for wtdd-planner when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-planner'), 'inherit');
  });

  test('returns "inherit" for wtdd-executor when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-executor'), 'inherit');
  });

  test('returns "inherit" for wtdd-phase-researcher when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-phase-researcher'), 'inherit');
  });

  test('returns "inherit" for wtdd-codebase-mapper when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-codebase-mapper'), 'inherit');
  });

  test('returns "inherit" for wtdd-verifier when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-verifier'), 'inherit');
  });
  ...
  test('does not silently fall back to "sonnet" (the original bug)', () => {
    ...
    const model = resolveModelInternal(tmpDir, 'wtdd-planner');
...
describe('bug #1829: model_profile "inherit" — resolve-model CLI', () => {
  ...
  test('CLI resolve-model returns "inherit" for wtdd-executor with inherit profile', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'inherit' }, null, 2)
    );

    const result = runGsdTools('resolve-model wtdd-executor', tmpDir);
  ...
  test('CLI resolve-model returns "inherit" for wtdd-planner with inherit profile', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'inherit' }, null, 2)
    );

    const result = runGsdTools('resolve-model wtdd-planner', tmpDir);
    assert.ok(result.success, `resolve-model failed: ${result.error}`);
