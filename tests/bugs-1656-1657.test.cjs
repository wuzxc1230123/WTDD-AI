...
describe('#1656: community .sh hooks must be present in hooks/dist', () => {
  ...
  test('wtdd-session-state.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'wtdd-session-state.sh');
    assert.ok(fs.existsSync(p), 'wtdd-session-state.sh must be in hooks/dist/ so the installer can copy it');
  });

  test('wtdd-validate-commit.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'wtdd-validate-commit.sh');
    assert.ok(fs.existsSync(p), 'wtdd-validate-commit.sh must be in hooks/dist/ so the installer can copy it');
  });

  test('wtdd-phase-boundary.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'wtdd-phase-boundary.sh');
    assert.ok(fs.existsSync(p), 'wtdd-phase-boundary.sh must be in hooks/dist/ so the installer can copy it');
  });
});
