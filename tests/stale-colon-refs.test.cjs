/**
 * Stale /wtdd: colon reference detection test
 *
...
/**
 * Determine whether a /wtdd: match in a test file is a legitimate test input
 * (i.e., the input string fed to a colon-to-hyphen converter).
 */
function isTestInput(filePath, line) {
  const rel = path.relative(ROOT, filePath).replace(/\\/g, '/');

  // SDK test files (.ts) that test sanitizer stripping of /wtdd: patterns
  ...
  // Conversion test files: input strings to convert* functions contain /wtdd:
  ...
  if (conversionTestFiles.includes(rel)) {
    ...
    // JSDoc block-comment lines with /wtdd: in description are stale
...
describe('No stale /wtdd: colon references (#1748)', () => {
  test('all /wtdd: references should be hyphenated except test inputs', () => {
    const extensions = new Set(['.md', '.js', '.cjs', '.ts', '.yml', '.sh', '.svg']);
    const files = collectFiles(ROOT, extensions);

    const staleRefs = [];
    const pattern = /\/wtdd:[a-z]/g;
    ...
    if (staleRefs.length > 0) {
      assert.fail(
        `Found ${staleRefs.length} stale /wtdd: colon reference(s) that should use /wtdd- hyphen format:\n${staleRefs.join('\n')}`
      );
    }
  });
});
