...
describe('Playwright-MCP UI verification integration', () => {
  test('verify-work.md mentions automated UI verification', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'verify-work.md'), 'utf-8'
    );
    assert.ok(
  ...
  test('ui-review.md mentions Playwright-MCP when available', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'ui-review.md'), 'utf-8'
    );
    assert.ok(
  ...
  test('wtdd-ui-auditor.md includes automated screenshot guidance', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'agents', 'wtdd', 'wtdd-ui-auditor.md'), 'utf-8'
    );
    assert.ok(
      content.toLowerCase().includes('playwright') || content.includes('screenshot') || content.includes('automated'),
      'wtdd-ui-auditor.md should mention automated screenshot verification'
    );
  });
