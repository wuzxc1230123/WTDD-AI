...
`
  ...
  describe('--cwd override', () => {
    ...
    beforeEach(() => {
      outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'wtdd-test-outside-'));
    });
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.wtdd_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.wtdd_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    assert.ok(content.includes('wtdd_state_version: 1.0'), 'should have version field');
...
const { stateExtractField, stateReplaceField, stateReplaceFieldWithFallback } = require('../what-the-dog-doing/bin/lib/state.cjs');
