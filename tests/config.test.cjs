/**
 * WTDD Tools Tests - config.cjs
 *
 * CLI integration tests for config-ensure-section, config-set, and config-get
 * commands exercised through wtdd-tools.cjs via execSync.
 *
...
describe('config-ensure-section command', () => {
  ...
  test('creates config.json with expected structure and types', () => {
    ...
    // Verify structure and types — exact values may vary if ~/.wtdd/defaults.json exists
  ...
  test('detects Brave Search from file-based key', () => {
    ...
    fs.writeFileSync(path.join(wtddDir, 'brave_api_key'), 'test-key', 'utf-8');
  ...
  test('merges user defaults from defaults.json', () => {
    ...
    const wtddDir = path.join(tmpDir, '.wtdd');
    fs.mkdirSync(wtddDir, { recursive: true });
    fs.writeFileSync(path.join(wtddDir, 'defaults.json'), JSON.stringify({
...
describe('config-new-project command', () => {
  ...
  test('creates full config with all expected keys', () => {
    ...
    assert.strictEqual(config.git.phase_branch_template, 'wtdd/phase-{phase}-{slug}');
    assert.strictEqual(config.git.milestone_branch_template, 'wtdd/{milestone}-{slug}');
...
describe('config-set/config-get context', () => {
  ...
  test('all three context profile files exist', () => {
    const contextsDir = path.join(__dirname, '..', 'what-the-dog-doing', 'contexts');
