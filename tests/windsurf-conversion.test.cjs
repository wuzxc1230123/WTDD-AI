...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'wtdd-quick');
...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'wtdd-plan-phase');
...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'wtdd-test');
...
describe('convertClaudeToWindsurfMarkdown', () => {
  ...
  test('replaces $ARGUMENTS with {{WTDD_ARGS}}', () => {
    const input = 'Pass $ARGUMENTS to the command.';
    const result = convertClaudeToWindsurfMarkdown(input);
    assert.ok(result.includes('{{WTDD_ARGS}}'), '$ARGUMENTS replaced');
  });
