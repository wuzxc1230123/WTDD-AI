/**
 * WTDD Tools Tests - discuss-phase power user mode
 *
...
describe('discuss-phase power user mode (#1513)', () => {
  const commandPath = path.join(__dirname, '..', 'commands', 'wtdd', 'discuss-phase.md');
  const workflowPath = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'discuss-phase.md');
  const powerWorkflowPath = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'discuss-phase-power.md');
  ...
  describe('command file (discuss-phase.md)', () => {
    test('mentions --power flag in argument-hint or description', () => {
      ...
      assert.ok(
        ...
        'commands/wtdd/discuss-phase.md should document the --power flag'
      );
    });
  ...
  describe('power workflow file (discuss-phase-power.md)', () => {
    test('file exists', () => {
      assert.ok(
        ...
        'what-the-dog-doing/workflows/discuss-phase-power.md should exist'
      );
    });
