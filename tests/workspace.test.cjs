/**
 * WTDD Workspace Tests
 *
 * Tests for /wtdd-new-workspace, /wtdd-list-workspaces, /wtdd-remove-workspace
 * init functions and integration with wtdd-tools routing.
 */
...
const { detectChildRepos } = require('../what-the-dog-doing/bin/lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('wtdd-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via wtdd-tools ──────────────────────────────────────
...
// ─── cmdInitListWorkspaces via wtdd-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'wtdd-workspaces');
...
// ─── cmdInitRemoveWorkspace via wtdd-tools ───────────────────────────────────
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('wtdd-ws-integ-');
...
describe('workspace command files', () => {
  ...
  test('new-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/wtdd/new-workspace.md'), 'utf8');
    assert.ok(content.includes('name: wtdd:new-workspace'));
  ...
  test('list-workspaces command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/wtdd/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('name: wtdd:list-workspaces'));
    assert.ok(content.includes('workflows/list-workspaces.md'));
  });

  test('remove-workspace command exists with correct frontmatter', () => {
    const content = fs.readFileSync(path.join(baseDir, 'commands/wtdd/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('name: wtdd:remove-workspace'));
    assert.ok(content.includes('workflows/remove-workspace.md'));
  });

  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'what-the-dog-doing/workflows/new-workspace.md'), 'utf8');
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'what-the-dog-doing/workflows/list-workspaces.md'), 'utf8');
    assert.ok(content.includes('init list-workspaces'));
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'what-the-dog-doing/workflows/remove-workspace.md'), 'utf8');
    assert.ok(content.includes('init remove-workspace'));
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in wtdd-tools ───────────────────────────────────────────────────

describe('workspace routing in wtdd-tools', () => {
  test('init new-workspace is routed correctly', () => {
    const toolsContent = fs.readFileSync(
      path.join(__dirname, '..', 'what-the-dog-doing', 'bin', 'wtdd-tools.cjs'),
      'utf8'
    );
