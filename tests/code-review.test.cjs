/**
 * WTDD Code Review Tests
 *
 * Validates all code review artifacts from Phases 1-4:
 * - Agent frontmatter (wtdd-code-reviewer, wtdd-code-fixer)
...
const AGENTS_DIR = path.join(__dirname, '..', 'agents', 'wtdd');
const COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'wtdd');
const WORKFLOWS_DIR = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows');
const CONFIG_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'bin', 'lib', 'config.cjs');

// Plugin directory resolution (cross-platform safe)
const PLUGIN_WORKFLOWS_DIR = process.env.WTDD_PLUGIN_ROOT || path.join(os.homedir(), '.claude', 'what-the-dog-doing', 'workflows');
...
describe('CR-AGENT: code review agent frontmatter', () => {
  test('wtdd-code-reviewer.md has required frontmatter fields', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name:'), 'wtdd-code-reviewer missing name:');
    assert.ok(frontmatter.includes('description:'), 'wtdd-code-reviewer missing description:');
    assert.ok(frontmatter.includes('tools:'), 'wtdd-code-reviewer missing tools:');
    assert.ok(frontmatter.includes('color:'), 'wtdd-code-reviewer missing color:');
  });

  test('wtdd-code-fixer.md has required frontmatter fields', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name:'), 'wtdd-code-fixer missing name:');
    assert.ok(frontmatter.includes('description:'), 'wtdd-code-fixer missing description:');
    assert.ok(frontmatter.includes('tools:'), 'wtdd-code-fixer missing tools:');
    assert.ok(frontmatter.includes('color:'), 'wtdd-code-fixer missing color:');
  });

  test('wtdd-code-reviewer.md has Read, Bash, Glob, Grep, Write tools', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('Read'), 'wtdd-code-reviewer missing Read tool');
    assert.ok(frontmatter.includes('Bash'), 'wtdd-code-reviewer missing Bash tool');
    assert.ok(frontmatter.includes('Glob'), 'wtdd-code-reviewer missing Glob tool');
    assert.ok(frontmatter.includes('Grep'), 'wtdd-code-reviewer missing Grep tool');
    assert.ok(frontmatter.includes('Write'), 'wtdd-code-reviewer missing Write tool');
  });

  test('wtdd-code-fixer.md has Read, Edit, Write, Bash, Grep, Glob tools', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('Read'), 'wtdd-code-fixer missing Read tool');
    assert.ok(frontmatter.includes('Edit'), 'wtdd-code-fixer missing Edit tool');
    assert.ok(frontmatter.includes('Write'), 'wtdd-code-fixer missing Write tool');
    assert.ok(frontmatter.includes('Bash'), 'wtdd-code-fixer missing Bash tool');
  });

  test('wtdd-code-reviewer.md does not have skills: in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(!frontmatter.includes('skills:'),
      'wtdd-code-reviewer has skills: in frontmatter — breaks Gemini CLI');
  });

  test('wtdd-code-fixer.md does not have skills: in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(!frontmatter.includes('skills:'),
      'wtdd-code-fixer has skills: in frontmatter — breaks Gemini CLI');
  });

  test('wtdd-code-fixer.md rollback uses git checkout (not Write tool)', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-fixer.md'), 'utf-8');
    assert.ok(content.includes('git checkout --'),
      'wtdd-code-fixer rollback should use git checkout -- {file} for atomic rollback');
    assert.ok(!content.includes('PRE_FIX_CONTENT'),
      'wtdd-code-fixer should not use PRE_FIX_CONTENT in-memory capture (use git checkout instead)');
  });

  test('wtdd-code-fixer.md success_criteria consistent with rollback strategy (git checkout)', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-fixer.md'), 'utf-8');
    ...
    assert.ok(successCriteria.includes('git checkout'),
      'wtdd-code-fixer success_criteria must reference git checkout rollback');
    assert.ok(!successCriteria.includes('Write tool with captured'),
      'wtdd-code-fixer success_criteria must not say Write tool for rollback');
  });

  test('wtdd-code-fixer.md flags logic-bug fixes for human review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-fixer.md'), 'utf-8');
    assert.ok(content.includes('requires human verification'),
      'wtdd-code-fixer should flag logic-bug fixes as requiring human verification');
  });

  test('wtdd-code-reviewer.md REVIEW.md spec includes files_reviewed_list field', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-code-reviewer.md'), 'utf-8');
    assert.ok(content.includes('files_reviewed_list'),
      'wtdd-code-reviewer REVIEW.md frontmatter spec must include files_reviewed_list for --auto scope persistence');
  });
});
...
describe('CR-CMD: code review command structure', () => {
  test('code-review.md has correct frontmatter name: wtdd:code-review', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'code-review.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name: wtdd:code-review'),
      'code-review.md missing correct name in frontmatter');
  });

  test('code-review-fix.md has correct frontmatter name: wtdd:code-review-fix', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'code-review-fix.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name: wtdd:code-review-fix'),
...
describe('CR-WORKFLOW: code review workflow structure', () => {
  ...
  test('code-review.md workflow references wtdd-code-reviewer agent', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'code-review.md'), 'utf-8');

    assert.ok(content.includes('wtdd-code-reviewer'),
      'code-review.md workflow does not reference wtdd-code-reviewer agent');
  });
  ...
  test('code-review-fix.md workflow references wtdd-code-fixer agent', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'code-review-fix.md'), 'utf-8');

    assert.ok(content.includes('wtdd-code-fixer'),
      'code-review-fix.md workflow does not reference wtdd-code-fixer agent');
  });
...
describe('CR-CONFIG: config key registration', () => {
  ...
  test('wtdd-tools config-get workflow.code_review succeeds', () => {
  ...
  test('wtdd-tools config-get workflow.code_review_depth succeeds', () => {
    const tmpDir = createTempProject();
...
describe('CR-INTEGRATION: workflow integration points', () => {
  ...
  test('autonomous.md contains wtdd:code-review skill invocation', { skip: !PLUGIN_AVAILABLE ? 'Plugin dir not installed' : false }, () => {
    const content = fs.readFileSync(path.join(PLUGIN_WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    assert.ok(content.includes('wtdd:code-review'),
      'autonomous.md missing wtdd:code-review skill invocation');
  });

  test('autonomous.md contains wtdd:code-review-fix skill invocation', { skip: !PLUGIN_AVAILABLE ? 'Plugin dir not installed' : false }, () => {
    const content = fs.readFileSync(path.join(PLUGIN_WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    assert.ok(content.includes('wtdd:code-review-fix'),
      'autonomous.md missing wtdd:code-review-fix skill invocation');
  });
