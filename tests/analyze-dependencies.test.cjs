...
describe('analyze-dependencies command', () => {
  test('command file exists', () => {
    const p = path.join(__dirname, '..', 'commands', 'wtdd', 'analyze-dependencies.md');
    assert.ok(fs.existsSync(p), 'commands/wtdd/analyze-dependencies.md should exist');
  });
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'analyze-dependencies.md');
