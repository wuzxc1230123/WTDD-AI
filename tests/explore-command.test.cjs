...
describe('explore command', () => {
  test('command file exists', () => {
    const p = path.join(__dirname, '..', 'commands', 'wtdd', 'explore.md');
    assert.ok(fs.existsSync(p), 'commands/wtdd/explore.md should exist');
  });

  test('command file has required frontmatter', () => {
    const p = path.join(__dirname, '..', 'commands', 'wtdd', 'explore.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: wtdd:explore'), 'Command must have name frontmatter');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'explore.md');
