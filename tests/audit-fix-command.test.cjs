...
describe('AUDIT-FIX: command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in commands/wtdd/'
    );
  });

  test('has valid frontmatter with name wtdd:audit-fix', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: wtdd:audit-fix'),
      'name must be wtdd:audit-fix'
    );
  });
  ...
  test('has argument-hint in frontmatter for /wtdd-help discoverability', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('argument-hint:'),
      'must have argument-hint in frontmatter for /wtdd-help discoverability'
    );
  });
...
describe('AUDIT-FIX: workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in what-the-dog-doing/workflows/'
    );
  });
  ...
  test('has <available_agent_types> listing wtdd-executor', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
    ...
    assert.ok(
      content.includes('wtdd-executor'),
      'must list wtdd-executor as available agent type'
    );
  });
});
