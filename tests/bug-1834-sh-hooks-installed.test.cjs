 ...
 * wtdd-session-state.sh, wtdd-validate-commit.sh, and wtdd-phase-boundary.sh
...
describe('#1834: installer deploys .sh hooks alongside .js hooks', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('wtdd-install-1834-');
  });
  ...
  test('wtdd-session-state.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'wtdd-session-state.sh');
    assert.ok(
      fs.existsSync(target),
      'wtdd-session-state.sh must be installed to hooks/ — missing file causes SessionStart hook errors'
    );
  });

  test('wtdd-validate-commit.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'wtdd-validate-commit.sh');
    assert.ok(
      fs.existsSync(target),
      'wtdd-validate-commit.sh must be installed to hooks/ — missing file causes PreToolUse hook errors'
    );
  });

  test('wtdd-phase-boundary.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'wtdd-phase-boundary.sh');
    assert.ok(
      fs.existsSync(target),
      'wtdd-phase-boundary.sh must be installed to hooks/ — missing file causes PostToolUse hook errors'
    );
  });
