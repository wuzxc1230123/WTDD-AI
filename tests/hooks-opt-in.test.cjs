/**
 * WTDD Tools Tests - Community Hooks (opt-in)
 *
...
function createTempProject(prefix = 'wtdd-hook-test-') {
...
describe('hook file validation', () => {
  test('wtdd-session-state.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'wtdd-session-state.sh');
    assert.ok(fs.existsSync(hookPath), 'wtdd-session-state.sh should exist');
  });

  test('wtdd-validate-commit.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'wtdd-validate-commit.sh');
    assert.ok(fs.existsSync(hookPath), 'wtdd-validate-commit.sh should exist');
  });

  test('wtdd-phase-boundary.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'wtdd-phase-boundary.sh');
    assert.ok(fs.existsSync(hookPath), 'wtdd-phase-boundary.sh should exist');
  });

  test('wtdd-session-state.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'wtdd-session-state.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'wtdd-session-state.sh should be executable');
  });

  test('wtdd-validate-commit.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'wtdd-validate-commit.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'wtdd-validate-commit.sh should be executable');
  });

  test('wtdd-phase-boundary.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'wtdd-phase-boundary.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'wtdd-phase-boundary.sh should be executable');
  });
});
...
describe('installer hook registration', () => {
  ...
  test('install.js contains wtdd-validate-commit registration block', () => {
    assert.ok(
      installSource.includes('wtdd-validate-commit'),
      'install.js should contain wtdd-validate-commit hook registration'
    );
    assert.ok(
  ...
  test('install.js contains wtdd-session-state registration block', () => {
    assert.ok(
      installSource.includes('wtdd-session-state'),
      'install.js should contain wtdd-session-state hook registration'
    );
    assert.ok(
  ...
  test('install.js contains wtdd-phase-boundary registration block', () => {
    assert.ok(
      installSource.includes('wtdd-phase-boundary'),
      'install.js should contain wtdd-phase-boundary hook registration'
    );
    assert.ok(
  ...
  test('install.js adds all 3 new hooks to the uninstall cleanup list', () => {
    const wtddHooksMatch = installSource.match(/const wtddHooks\s*=\s*\[([^\]]+)\]/);
    assert.ok(wtddHooksMatch, 'install.js should define wtddHooks array for uninstall cleanup');

    const wtddHooksContent = wtddHooksMatch[1];
    assert.ok(
      wtddHooksContent.includes('wtdd-session-state.sh'),
      'wtddHooks should include wtdd-session-state.sh'
    );
    assert.ok(
      wtddHooksContent.includes('wtdd-validate-commit.sh'),
      'wtddHooks should include wtdd-validate-commit.sh'
    );
    assert.ok(
      wtddHooksContent.includes('wtdd-phase-boundary.sh'),
      'wtddHooks should include wtdd-phase-boundary.sh'
    );
  });
...
describe('opt-in gating behavior', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('validate-commit is a no-op when config.json is absent', (t) => {
    ...
    const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-hook-bare-'));
...
describe('hook execution when enabled', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('session-state exits 0 without .planning/ (in enabled project)', (t) => {
    ...
    const noStateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-hook-nostate-'));
