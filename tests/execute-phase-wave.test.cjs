 ...
 *
 * Validates the /wtdd-execute-phase --wave feature contract:
...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'wtdd', 'execute-phase.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'execute-phase.md');
const COMMANDS_DOC_PATH = path.join(__dirname, '..', 'docs', 'COMMANDS.md');
const HELP_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'help.md');

describe('execute-phase command: --wave flag', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/wtdd/execute-phase.md should exist');
  });
...
describe('execute-phase docs: user-facing wave flag', () => {
  test('COMMANDS.md documents --wave usage', () => {
    ...
    assert.ok(
      content.includes('/wtdd-execute-phase 1 --wave 2'),
  ...
  test('help workflow documents --wave behavior', () => {
    ...
    assert.ok(
      content.includes('Usage: `/wtdd-execute-phase 5 --wave 2`'),
...
describe('use_worktrees config: cross-workflow structural coverage', () => {
  const QUICK_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'quick.md');
  const DIAGNOSE_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'diagnose-issues.md');
  const EXECUTE_PLAN_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'execute-plan.md');
  const PLANNING_CONFIG_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'references', 'planning-config.md');
  const CONFIG_CJS_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'bin', 'lib', 'config.cjs');
