/**
 * WTDD Tools Tests - Validate Health Command
 *
...
describe('validate health --repair command', () => {
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'wtdd/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'wtdd/{milestone}-{slug}');
  });
