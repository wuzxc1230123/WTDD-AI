/**
 * Regression test for #1860: detectConfigDir in wtdd-check-update.js should
...
describe('detectConfigDir runtime behavior (#1860)', () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-test-home-'));
  });
  ...
  test('returns .claude config dir when both .claude and .config/opencode exist', () => {
    ...
    const openCodeVersionDir = path.join(tmpHome, '.config', 'opencode', 'what-the-dog-doing');
    ...
    const claudeVersionDir = path.join(tmpHome, '.claude', 'what-the-dog-doing');
