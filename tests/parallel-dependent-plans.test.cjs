 ...
 *
 * Validates that:
 * 1. wtdd-planner.md assign_waves step explicitly checks files_modified overlap
...
const PLANNER_AGENT_PATH = path.join(__dirname, '..', 'agents', 'wtdd', 'wtdd-planner.md');
...
// ---------------------------------------------------------------------------
// wtdd-planner.md — wave assignment must account for files_modified overlap
// ---------------------------------------------------------------------------

describe('wtdd-planner agent: files_modified wave ordering', () => {
  test('planner agent file exists', () => {
    assert.ok(fs.existsSync(PLANNER_AGENT_PATH), 'agents/wtdd-planner.md should exist');
  });
  ...
  test('assign_waves step contains explicit overlap → later-wave rule', () => {
    ...
    assert.ok(assignWavesMatch, 'assign_waves step should exist in wtdd-planner.md');
