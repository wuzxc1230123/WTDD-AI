/**
 * WTDD Agent Installation Validation Tests (#1371)
 *
 * Validates that WTDD detects missing or incomplete agent installations and
 * surfaces warnings through init commands and health checks. When agents are
 * not installed, Task(subagent_type="wtdd-*") silently falls back to
...
const MODEL_PROFILES = require('../what-the-dog-doing/bin/lib/model-profiles.cjs').MODEL_PROFILES;
const EXPECTED_AGENTS = Object.keys(MODEL_PROFILES);

/**
 * Create a fake WTDD install directory structure that mirrors what the installer
 * produces. wtdd-tools.cjs lives at <configDir>/what-the-dog-doing/bin/wtdd-tools.cjs,
 * so the agents dir is at <configDir>/agents/.
 *
 * We use --cwd to point at the project, and WTDD_INSTALL_DIR env to override
...
describe('init commands: agents_installed field (#1371)', () => {
  ...
  test('init execute-phase includes agents_installed=true when agents exist', () => {
    ...
    // Create agents dir as sibling of what-the-dog-doing/ (the installed layout)
    // wtdd-tools.cjs resolves agents from WTDD_INSTALL_DIR or __dirname/../../agents
    const wtddInstallDir = path.resolve(__dirname, '..', 'what-the-dog-doing', 'bin');
    const configDir = path.resolve(wtddInstallDir, '..', '..');
    const agentsDir = path.join(configDir, 'agents');

    // Agents already exist in the repo root /agents/ dir which is sibling to what-the-dog-doing/
    ...
    // The repo has agents/ dir with all wtdd-*.md files, so this should be true
    assert.strictEqual(output.agents_installed, true,
      'agents_installed should be true when agents directory has wtdd-*.md files');
  });
...
describe('validate health: agent installation check W010 (#1371)', () => {
  ...
  test('health check reports healthy when agents are installed (repo layout)', () => {
    // In the repo, agents/ exists as a sibling of what-the-dog-doing/, so the
    // health check should find them via the wtdd-tools.cjs path resolution
...
describe('checkAgentsInstalled: Copilot .agent.md format (#1512)', () => {
  ...
  test('agents_installed=true when agents exist as .agent.md (Copilot format)', () => {
    // Simulate a Copilot install: agents are named wtdd-*.agent.md, not wtdd-*.md
    // Use WTDD_AGENTS_DIR to point at an isolated dir with ONLY .agent.md files,
    ...
    const result = runGsdTools('validate agents --raw', tmpDir, { WTDD_AGENTS_DIR: agentsDir });
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    // Must report the custom dir, not the default repo agents dir
    assert.strictEqual(output.agents_dir, agentsDir,
      'agents_dir must be the WTDD_AGENTS_DIR override, not the repo default');
  ...
  test('init new-workspace includes agents_installed=true with Copilot .agent.md files', () => {
    ...
    const result = runGsdTools('init new-workspace --raw', tmpDir, { WTDD_AGENTS_DIR: agentsDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('WTDD_AGENTS_DIR env var overrides default agents directory', () => {
    ...
    const result = runGsdTools('validate agents --raw', tmpDir, { WTDD_AGENTS_DIR: customAgentsDir });
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    // The custom dir path should be reported
    assert.strictEqual(output.agents_dir, customAgentsDir,
      'agents_dir must reflect WTDD_AGENTS_DIR override');
  });
});
