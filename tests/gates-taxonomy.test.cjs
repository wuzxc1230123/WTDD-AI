...
const GATES_REF = path.join(ROOT, 'what-the-dog-doing', 'references', 'gates.md');

describe('gates taxonomy (#1715)', () => {
  test('reference file exists', () => {
    assert.ok(
      fs.existsSync(GATES_REF),
      'what-the-dog-doing/references/gates.md must exist'
    );
  });
  ...
  test('plan-phase.md references gates.md', () => {
    const planPhase = path.join(ROOT, 'what-the-dog-doing', 'workflows', 'plan-phase.md');
  ...
  test('execute-phase.md references gates.md', () => {
    const execPhase = path.join(ROOT, 'what-the-dog-doing', 'workflows', 'execute-phase.md');
  ...
  test('wtdd-plan-checker.md references gates.md in required_reading block', () => {
    const planChecker = path.join(ROOT, 'agents', 'wtdd', 'wtdd-plan-checker.md');
    const content = fs.readFileSync(planChecker, 'utf-8');
    assert.ok(
      content.includes('<required_reading>'),
      'wtdd-plan-checker.md must have a <required_reading> block'
    );
    const reqBlock = content.split('<required_reading>')[1].split('</required_reading>')[0];
    assert.ok(
      reqBlock.includes('references/gates.md'),
      'wtdd-plan-checker.md must reference gates.md inside <required_reading>'
    );
  });

  test('wtdd-verifier.md references gates.md in required_reading block', () => {
    const verifier = path.join(ROOT, 'agents', 'wtdd', 'wtdd-verifier.md');
    const content = fs.readFileSync(verifier, 'utf-8');
    assert.ok(
      content.includes('<required_reading>'),
      'wtdd-verifier.md must have a <required_reading> block'
    );
    const reqBlock = content.split('<required_reading>')[1].split('</required_reading>')[0];
    assert.ok(
      reqBlock.includes('references/gates.md'),
      'wtdd-verifier.md must reference gates.md inside <required_reading>'
    );
  });
