...
describe('Cline runtime support', () => {
  ...
  test('.clinerules references WTDD workflow enforcement', () => {
    const content = fs.readFileSync(path.join(__dirname, '..', '.clinerules'), 'utf-8');
    assert.ok(
      content.includes('wtdd') || content.includes('WTDD') || content.includes('workflow'),
      '.clinerules should mention WTDD workflows'
    );
  });
