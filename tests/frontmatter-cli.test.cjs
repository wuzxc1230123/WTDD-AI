/**
 * WTDD Tools Tests - frontmatter CLI integration
 *
 * Integration tests for the 4 frontmatter subcommands (get, set, merge, validate)
 * exercised through wtdd-tools.cjs via execSync.
 *
...
function writeTempFile(content) {
  const tmpFile = path.join(os.tmpdir(), `wtdd-fm-test-${Date.now()}-${Math.random().toString(36).slice(2)}.md`);
...
describe('frontmatter set', () => {
  test('updates existing field', () => {
    ...
    const { extractFrontmatter } = require('../what-the-dog-doing/bin/lib/frontmatter.cjs');
