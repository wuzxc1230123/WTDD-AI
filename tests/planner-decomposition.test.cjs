/**
 * Tests for modular decomposition of agents/wtdd-planner.md
 *
 * Verifies that:
 *   1. wtdd-planner.md stays under the 100K agent file threshold
 *   2. wtdd-planner.md is under 45K chars (proving the three mode sections were extracted)
 *   3. The three reference files exist
 *   4. wtdd-planner.md contains reference pointers to each extracted file
...
const PLANNER_PATH = path.join(PROJECT_ROOT, 'agents', 'wtdd', 'wtdd-planner.md');
const GAP_CLOSURE_REF = path.join(PROJECT_ROOT, 'what-the-dog-doing', 'references', 'planner-gap-closure.md');
const REVISION_REF = path.join(PROJECT_ROOT, 'what-the-dog-doing', 'references', 'planner-revision.md');
const REVIEWS_REF = path.join(PROJECT_ROOT, 'what-the-dog-doing', 'references', 'planner-reviews.md');

// ─── wtdd-planner.md size ─────────────────────────────────────────────────────

describe('wtdd-planner.md size constraints', () => {
  ...
  test('planner is under 100K chars (agent file threshold)', () => {
    ...
    assert.ok(
      ...
      `wtdd-planner.md is ${content.length} chars, exceeds 100K agent threshold`
    );
  });
  ...
  test('planner is under 45K chars (proves mode sections were extracted)', () => {
    ...
    assert.ok(
      ...
      `wtdd-planner.md is ${content.length} chars, expected < 45K after extracting mode sections`
    );
  });
});
...
// ─── wtdd-planner.md contains reference pointers ──────────────────────────────

describe('wtdd-planner.md contains reference pointers to extracted files', () => {
  let plannerContent;
  ...
  test('planner references planner-gap-closure.md', () => {
    ...
    assert.ok(
      ...
      'wtdd-planner.md must reference planner-gap-closure.md'
    );
  });
  ...
  test('planner references planner-revision.md', () => {
    ...
    assert.ok(
      ...
      'wtdd-planner.md must reference planner-revision.md'
    );
  });
  ...
  test('planner references planner-reviews.md', () => {
    ...
    assert.ok(
      ...
      'wtdd-planner.md must reference planner-reviews.md'
    );
  });
});
