...
const commandPath = path.join(repoRoot, 'commands', 'wtdd', 'milestone-summary.md');
const workflowPath = path.join(repoRoot, 'what-the-dog-doing', 'workflows', 'milestone-summary.md');

describe('milestone-summary command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/wtdd/milestone-summary.md should exist');
  });

  test('command has correct frontmatter name', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: wtdd:milestone-summary'), 'should have correct command name');
  });
...
describe('milestone-summary workflow', () => {
  ...
  test('workflow updates STATE.md', () => {
    ...
    assert.ok(
      ...
      'should update STATE.md via wtdd-tools'
    );
  });
...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-ms-test-'));
  });
