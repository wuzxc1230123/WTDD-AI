...
describe('scan command', () => {
  test('command file exists with correct name and description', () => {
    const p = path.join(__dirname, '..', 'commands', 'wtdd', 'scan.md');
    assert.ok(fs.existsSync(p), 'commands/wtdd/scan.md should exist');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: wtdd:scan'), 'Command must have name: wtdd:scan');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'scan.md');
    assert.ok(fs.existsSync(p), 'what-the-dog-doing/workflows/scan.md should exist');
  });
  ...
  test('workflow references wtdd-codebase-mapper', () => {
    const p = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'scan.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('wtdd-codebase-mapper'),
      'Workflow should reference the wtdd-codebase-mapper agent');
  });
});
