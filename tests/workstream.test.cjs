...
function createFailingTtyEnv(tmpDir) {
  ...
  fs.writeFileSync(
    ...
    '#!/bin/sh\nif [ -n "$WTDD_TTY_MARKER" ]; then printf "tty\\n" >> "$WTDD_TTY_MARKER"; fi\nexit 99\n',
    'utf-8'
  );
  fs.chmodSync(path.join(binDir, 'tty'), 0o755);
  fs.writeFileSync(
    path.join(binDir, 'tty.cmd'),
    '@echo off\r\nif not "%WTDD_TTY_MARKER%"=="" echo tty>>"%WTDD_TTY_MARKER%"\r\nexit /b 99\r\n',
    'utf-8'
  );
...
function getSessionPointerDir(tmpDir) {
  ...
  const projectId = crypto
    ...
    .digest('hex')
    .slice(0, 16);
  return path.join(os.tmpdir(), 'wtdd-workstream-sessions', projectId);
}
...
describe('planningDir workstream awareness via env var', () => {
  ...
  test('state json returns workstream-scoped state when WTDD_WORKSTREAM is set', () => {
    const result = runGsdTools(['state', 'json', '--raw'], tmpDir, { WTDD_WORKSTREAM: 'alpha' });
  ...
  test('state json reads from flat .planning when no workstream set', () => {
    ...
    const result = runGsdTools(['state', 'json', '--raw'], tmpDir, { WTDD_WORKSTREAM: '' });
  ...
  test('--ws flag overrides WTDD_WORKSTREAM env var', () => {
    ...
    const result = runGsdTools(['state', 'json', '--raw', '--ws', 'beta'], tmpDir, { WTDD_WORKSTREAM: 'alpha' });
...
describe('session-scoped active workstream routing', () => {
  ...
  test('stores active workstream per session instead of mutating shared pointer', () => {
    const alphaSet = runGsdTools(['workstream', 'set', 'alpha', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    const betaSet = runGsdTools(['workstream', 'set', 'beta', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-beta' });
  ...
  test('different sessions resolve different active workstreams without --ws', () => {
    const alpha = runGsdTools(['workstream', 'get', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    const beta = runGsdTools(['workstream', 'get', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-beta' });
  ...
  test('state commands route to the session-scoped workstream automatically', () => {
    const alpha = runGsdTools(['state', 'json', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    const beta = runGsdTools(['state', 'json', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-beta' });
  ...
  test('clearing one session does not clear another session pointer', () => {
    const clearAlpha = runGsdTools(['workstream', 'set', '--clear', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    const alpha = runGsdTools(['workstream', 'get'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    const beta = runGsdTools(['workstream', 'get', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-beta' });
...
describe('session resolution hardening', () => {
  ...
  test('explicit runtime session ids outrank tty-derived identities', () => {
    const set = runGsdTools(['workstream', 'set', 'alpha', '--raw'], tmpDir, {
      WTDD_SESSION_KEY: 'shared-session',
      TTY: '/dev/pts/42',
    });
...
describe('pointer lifecycle hardening', () => {
  ...
  test('clearing one session pointer leaves sibling session pointers intact', () => {
    ...
    const alphaFile = getSessionPointerFileName('WTDD_SESSION_KEY', 'session-alpha');
    const betaFile = getSessionPointerFileName('WTDD_SESSION_KEY', 'session-beta');

    runGsdTools(['workstream', 'set', 'alpha', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    runGsdTools(['workstream', 'set', 'beta', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-beta' });
  ...
  test('clearing the last session pointer removes the empty session tmp directory', () => {
    const sessionDir = getSessionPointerDir(tmpDir);
    const set = runGsdTools(['workstream', 'set', 'alpha', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
    ...
    const clear = runGsdTools(['workstream', 'set', '--clear', '--raw'], tmpDir, { WTDD_SESSION_KEY: 'session-alpha' });
...
// ─── Integration: wtdd-tools --ws flag ────────────────────────────────────────

describe('wtdd-tools --ws flag integration', () => {
  let tmpDir;
...
describe('path traversal rejection', () => {
  ...
  describe('WTDD_WORKSTREAM env var rejects traversal attempts', () => {
    for (const name of maliciousNames) {
      test(`rejects WTDD_WORKSTREAM=${name}`, () => {
        const result = runGsdTools(['workstream', 'list', '--raw'], tmpDir, { WTDD_WORKSTREAM: name });
        assert.ok(!result.success, `should reject WTDD_WORKSTREAM=${name}`);
        assert.ok(result.error.includes('Invalid workstream name'), `error should mention invalid name for: ${name}`);
      });
    }
  });
  ...
  describe('getActiveWorkstream rejects poisoned active-workstream file', () => {
    for (const name of maliciousNames) {
      test(`rejects poisoned file containing ${name}`, () => {
        ...
        const result = runGsdTools(['workstream', 'get'], tmpDir, { WTDD_WORKSTREAM: '' });
  ...
  describe('setActiveWorkstream rejects invalid names directly', () => {
    const { setActiveWorkstream } = require('../what-the-dog-doing/bin/lib/core.cjs');
    for (const name of maliciousNames) {
