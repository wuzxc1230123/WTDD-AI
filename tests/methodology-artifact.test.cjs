...
const REFERENCES_DIR = path.join(ROOT, 'what-the-dog-doing', 'references');
const WORKFLOWS_DIR = path.join(ROOT, 'what-the-dog-doing', 'workflows');
...
describe('methodology artifact type (#1488)', () => {
  ...
  test('artifact-types.md exists in what-the-dog-doing/references/', () => {
    const p = path.join(REFERENCES_DIR, 'artifact-types.md');
    assert.ok(fs.existsSync(p), 'what-the-dog-doing/references/artifact-types.md must exist');
