/**
 * WTDD Tools Tests - Copilot Install Plumbing
 *
...
describe('convertClaudeToCopilotContent', () => {
  ...
  test('converts wtdd: to wtdd- in command names', () => {
    assert.strictEqual(
      convertClaudeToCopilotContent('run /wtdd:health or wtdd:progress'),
      'run /wtdd-health or wtdd-progress'
    );
  });
  ...
  test('handles mixed content in local mode', () => {
    const input = 'Config at ~/.claude/settings and $HOME/.claude/config.\n' +
      ...
      'Run wtdd:health and /wtdd:progress.';
    ...
    assert.ok(result.includes('wtdd-health'), 'command name converted');
    assert.ok(result.includes('/wtdd-progress'), 'slash command converted');
  });
...
Body content here referencing ~/.claude/foo and wtdd:health.`;

    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-health');
    ...
    assert.ok(result.includes('wtdd-health'), 'CONV-07 applied to body');
    assert.ok(!result.includes('wtdd:health'), 'no wtdd: references remain');
  });

  test('handles skill without allowed-tools', () => {
    const input = `---
name: wtdd:help
description: Show available WTDD commands
---

Help content.`;

    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-help');
    assert.ok(result.includes('name: wtdd-help'), 'name set');
    assert.ok(result.includes('description: Show available WTDD commands'), 'description preserved');
...
Progress body.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-progress');
...
Body.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-new-milestone');
...
Check ~/.claude/settings and ./.claude/local and $HOME/.claude/global.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-test');
    assert.ok(result.includes('.github/settings'), 'tilde path converted to local');
...
Check ~/.claude/settings and ./.claude/local and $HOME/.claude/global.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-test', true);
    assert.ok(result.includes('~/.copilot/settings'), 'tilde path converted to global');
    assert.ok(result.includes('./.github/local'), 'dot-slash path converted');
...
Run wtdd:health and /wtdd:progress for diagnostics.`;

    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-test');
    assert.ok(result.includes('wtdd-health'), 'wtdd:health converted');
    assert.ok(result.includes('/wtdd-progress'), '/wtdd:progress converted');
    assert.ok(!result.match(/wtdd:[a-z]/), 'no wtdd: command refs remain');
  });

  test('handles content without frontmatter (local mode)', () => {
    const input = 'Just some markdown with ~/.claude/path and wtdd:health.';
    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-test');
    assert.ok(result.includes('.github/path'), 'CONV-06 applied (local)');
    assert.ok(result.includes('wtdd-health'), 'CONV-07 applied');
    assert.ok(!result.includes('---'), 'no frontmatter added');
  });

  test('preserves agent field in frontmatter', () => {
    const input = `---
name: wtdd:execute-phase
...
Body.`;
    ...
    const result = convertClaudeCommandToCopilotSkill(input, 'wtdd-execute-phase');
    assert.ok(result.includes('agent: wtdd-planner'), 'agent field preserved');
  });
});
...
describe('convertClaudeAgentToCopilotAgent', () => {
  test('maps and deduplicates tools', () => {
    const input = `---
name: wtdd-executor
description: Executes WTDD plans
...
Body.`;
    ...
    const result = convertClaudeAgentToCopilotAgent(input);
    assert.ok(result.includes('name: wtdd-executor'), 'name preserved');
    assert.ok(result.includes('description: Executes WTDD plans with atomic commits'), 'description preserved');
    assert.ok(result.includes('color: yellow'), 'color preserved');
  });

  test('handles mcp__context7__ tools', () => {
    const input = `---
name: wtdd-researcher
...
Check ~/.claude/settings and run wtdd:health.`;
...
Check ~/.claude/settings and run wtdd:health.`;
  ...
  test('handles content without frontmatter (local mode)', () => {
    const input = 'Just markdown with ~/.claude/path and wtdd:test.';
    const result = convertClaudeAgentToCopilotAgent(input);
    assert.ok(result.includes('.github/path'), 'CONV-06 applied (local)');
    assert.ok(result.includes('wtdd-test'), 'CONV-07 applied');
    assert.ok(!result.includes('---'), 'no frontmatter added');
  });
});
...
describe('copyCommandsAsCopilotSkills', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-copilot-skills-'));
  });
  ...
  test('creates skill folders from source commands', () => {
    copyCommandsAsCopilotSkills(srcDir, tempDir, 'wtdd');

    // Check specific folders exist
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-health')), 'wtdd-health folder exists');
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-health', 'SKILL.md')), 'wtdd-health/SKILL.md exists');
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-help')), 'wtdd-help folder exists');
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-progress')), 'wtdd-progress folder exists');

    // Count wtdd-* directories — should match number of source command files
    const dirs = fs.readdirSync(tempDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('wtdd-'));
    const expectedSkillCount = fs.readdirSync(path.join(__dirname, '..', 'commands', 'wtdd'))
  ...
  test('skill content has Copilot frontmatter format', () => {
    ...
    const skillContent = fs.readFileSync(path.join(tempDir, 'wtdd-health', 'SKILL.md'), 'utf8');
    // Frontmatter format checks
    assert.ok(skillContent.startsWith('---\nname: wtdd-health\n'), 'starts with name: wtdd-health');
    ...
    assert.ok(!skillContent.match(/wtdd:[a-z]/), 'no wtdd: command references');
  });

  test('generates wtdd-autonomous skill from autonomous.md command', () => {
    // Fail-fast: source command must exist
    const srcFile = path.join(srcDir, 'autonomous.md');
    assert.ok(fs.existsSync(srcFile), 'commands/wtdd/autonomous.md must exist as source');

    copyCommandsAsCopilotSkills(srcDir, tempDir, 'wtdd');

    // Skill folder and file created
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-autonomous')), 'wtdd-autonomous folder exists');
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-autonomous', 'SKILL.md')), 'wtdd-autonomous/SKILL.md exists');

    const skillContent = fs.readFileSync(path.join(tempDir, 'wtdd-autonomous', 'SKILL.md'), 'utf8');

    // Frontmatter: name converted from wtdd:autonomous to wtdd-autonomous
    assert.ok(skillContent.startsWith('---\nname: wtdd-autonomous\n'), 'name is wtdd-autonomous');
  ...
  test('autonomous skill body converts wtdd: to wtdd- (CONV-07)', () => {
    ...
    // wtdd:autonomous references should be converted to wtdd-autonomous
    assert.ok(!result.match(/wtdd:[a-z]/), 'no wtdd: command references remain after conversion');
    // Specific: wtdd:discuss-phase, wtdd:plan-phase, wtdd:execute-phase mentioned in body
    // The body references wtdd-tools.cjs (not a wtdd: command) — those should be unaffected
    // But /wtdd:autonomous → /wtdd-autonomous, wtdd:discuss-phase → wtdd-discuss-phase etc.
    if (srcContent.includes('wtdd:autonomous')) {
      assert.ok(result.includes('wtdd-autonomous'), 'wtdd:autonomous converted to wtdd-autonomous');
    }
  ...
  test('cleans up old skill directories on re-run', () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'wtdd-fake-old'), { recursive: true });
    fs.writeFileSync(path.join(tempDir, 'wtdd-fake-old', 'SKILL.md'), 'old');
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-fake-old')), 'fake old dir exists before');

    // Run copy — should clean up old dirs
    copyCommandsAsCopilotSkills(srcDir, tempDir, 'wtdd');

    assert.ok(!fs.existsSync(path.join(tempDir, 'wtdd-fake-old')), 'fake old dir removed');
    assert.ok(fs.existsSync(path.join(tempDir, 'wtdd-health')), 'real dirs still exist');
  });
});
...
describe('Copilot agent conversion - real files', () => {
  const agentsSrc = path.join(__dirname, '..', 'agents', 'wtdd');

  test('converts wtdd-executor agent correctly', () => {
    const content = fs.readFileSync(path.join(agentsSrc, 'wtdd-executor.md'), 'utf8');
  ...
  test('converts agent with mcp wildcard tools correctly', () => {
    const content = fs.readFileSync(path.join(agentsSrc, 'wtdd-phase-researcher.md'), 'utf8');
    const result = convertClaudeAgentToCopilotAgent(content);
  ...
  test('all 18 agents convert without error', () => {
    const agents = fs.readdirSync(agentsSrc)
      .filter(f => f.startsWith('wtdd-') && f.endsWith('.md'));
    const expectedAgentCount = fs.readdirSync(agentsSrc)
      .filter(f => f.startsWith('wtdd-') && f.endsWith('.md')).length;
...
describe('Copilot content conversion - engine files', () => {
  test('converts engine .md files correctly (local mode default)', () => {
    const healthMd = fs.readFileSync(
      path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'health.md'), 'utf8'
    );
    ...
    assert.ok(!result.match(/\/wtdd:[a-z]/), 'no /wtdd: command references remain');
    assert.ok(!result.match(/(?<!\/)wtdd:[a-z]/), 'no bare wtdd: command references remain');
  ...
  test('converts engine .cjs files correctly', () => {
    const verifyCjs = fs.readFileSync(
      path.join(__dirname, '..', 'what-the-dog-doing', 'bin', 'lib', 'verify.cjs'), 'utf8'
    );
    const result = convertClaudeToCopilotContent(verifyCjs);

    assert.ok(!result.match(/wtdd:[a-z]/), 'no wtdd: references remain');
    assert.ok(result.includes('wtdd-new-project'), 'wtdd:new-project converted');
    assert.ok(result.includes('wtdd-health'), 'wtdd:health converted');
  });
});
...
describe('Copilot instructions merge/strip', () => {
  ...
  const wtddContent = '- Follow project conventions\n- Use structured workflows';

  function makeGsdBlock(content) {
    return WTDD_COPILOT_INSTRUCTIONS_MARKER + '\n' + content.trim() + '\n' + WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER;
  }
  ...
  describe('mergeCopilotInstructions', () => {
    ...
    beforeEach(() => {
      tmpMergeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-merge-'));
    });
    ...
    test('creates file from scratch when none exists', () => {
      ...
      mergeCopilotInstructions(filePath, wtddContent);

      assert.ok(fs.existsSync(filePath), 'file was created');
      const result = fs.readFileSync(filePath, 'utf8');
      assert.ok(result.includes(WTDD_COPILOT_INSTRUCTIONS_MARKER), 'has opening marker');
      assert.ok(result.includes(WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER), 'has closing marker');
      assert.ok(result.includes('Follow project conventions'), 'has WTDD content');
    });

    test('replaces WTDD section when both markers present', () => {
      const filePath = path.join(tmpMergeDir, 'copilot-instructions.md');
      const oldContent = '# User Setup\n\n' +
        makeGsdBlock('- Old WTDD content') +
        '\n\n# User Notes\n';
      ...
      assert.ok(!result.includes('Old WTDD content'), 'old WTDD content removed');
      assert.ok(result.includes('Follow project conventions'), 'new WTDD content inserted');
    });
    ...
    test('appends to existing file when no markers present', () => {
      ...
      assert.ok(result.includes(WTDD_COPILOT_INSTRUCTIONS_MARKER), 'WTDD block appended');
      assert.ok(result.includes('Follow project conventions'), 'WTDD content appended');
      // Verify separator exists
      assert.ok(result.includes('Do things my way.\n\n' + WTDD_COPILOT_INSTRUCTIONS_MARKER),
        'double newline separator before WTDD block');
    });

    test('handles file that is WTDD-only (re-creates cleanly)', () => {
      const filePath = path.join(tmpMergeDir, 'copilot-instructions.md');
      const wtddOnly = makeGsdBlock('- Old instructions') + '\n';
      fs.writeFileSync(filePath, wtddOnly);
    ...
    test('preserves user content before and after markers', () => {
      ...
      assert.ok(result.includes('Follow project conventions'), 'new WTDD content between markers');
      // Verify ordering: before → WTDD → after
      const setupIdx = result.indexOf('# My Setup');
      const markerIdx = result.indexOf(WTDD_COPILOT_INSTRUCTIONS_MARKER);
      const notesIdx = result.indexOf('# My Notes');
      assert.ok(setupIdx < markerIdx, 'user setup comes before WTDD block');
      assert.ok(markerIdx < notesIdx, 'WTDD block comes before user notes');
    });
  });

  describe('stripGsdFromCopilotInstructions', () => {
    test('returns null when content is WTDD-only', () => {
      const content = makeGsdBlock('- WTDD instructions only') + '\n';
      const result = stripGsdFromCopilotInstructions(content);
      assert.strictEqual(result, null, 'returns null for WTDD-only content');
    });

    test('returns cleaned content when user content exists before markers', () => {
      const content = '# My Setup\n\nCustom rules here.\n\n' +
        makeGsdBlock('- WTDD stuff') + '\n';
      const result = stripGsdFromCopilotInstructions(content);
      ...
      assert.ok(!result.includes(WTDD_COPILOT_INSTRUCTIONS_MARKER), 'opening marker removed');
      assert.ok(!result.includes(WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER), 'closing marker removed');
      assert.ok(!result.includes('WTDD stuff'), 'WTDD content removed');
    });

    test('returns cleaned content when user content exists after markers', () => {
      const content = makeGsdBlock('- WTDD stuff') + '\n\n# My Notes\n\nPersonal notes.\n';
      const result = stripGsdFromCopilotInstructions(content);
...
describe('Copilot uninstall skill removal', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-uninstall-'));
  });
  ...
  test('identifies wtdd-* skill directories for removal', () => {
    // Create Copilot-like skills directory structure
    const skillsDir = path.join(tmpDir, 'skills');
    fs.mkdirSync(path.join(skillsDir, 'wtdd-foo'), { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'wtdd-foo', 'SKILL.md'), '# Foo');
    fs.mkdirSync(path.join(skillsDir, 'wtdd-bar'), { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'wtdd-bar', 'SKILL.md'), '# Bar');
    ...
    // Test the pattern: read skills, filter wtdd-* entries
    const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    const wtddSkills = entries
      .filter(e => e.isDirectory() && e.name.startsWith('wtdd-'))
      .map(e => e.name);
    ...
    assert.deepStrictEqual(wtddSkills.sort(), ['wtdd-bar', 'wtdd-foo'], 'identifies wtdd-* skills');
    assert.deepStrictEqual(nonGsdSkills, ['custom-skill'], 'preserves non-wtdd skills');
  });

  test('cleans WTDD section from copilot-instructions.md on uninstall', () => {
    const content = '# My Setup\n\nMy custom rules.\n\n' +
      WTDD_COPILOT_INSTRUCTIONS_MARKER + '\n' +
      '- WTDD managed content\n' +
      WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER + '\n';
    ...
    assert.ok(!result.includes('WTDD managed content'), 'WTDD content removed');
    assert.ok(!result.includes(WTDD_COPILOT_INSTRUCTIONS_MARKER), 'markers removed');
  });

  test('deletes copilot-instructions.md when WTDD-only on uninstall', () => {
    const content = WTDD_COPILOT_INSTRUCTIONS_MARKER + '\n' +
      '- Only WTDD content\n' +
      WTDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER + '\n';
...
describe('Copilot manifest and patches fixes', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-manifest-'));
  });
  ...
  test('writeManifest hashes skills for Copilot runtime', () => {
    // Create minimal what-the-dog-doing dir (required by writeManifest)
    const wtddDir = path.join(tmpDir, 'what-the-dog-doing', 'bin');
    fs.mkdirSync(wtddDir, { recursive: true });
    fs.writeFileSync(path.join(wtddDir, 'verify.cjs'), '// verify stub');

    // Create Copilot skills directory
    const skillDir = path.join(tmpDir, 'skills', 'wtdd-test');
    ...
    const skillKey = 'skills/wtdd-test/SKILL.md';
  ...
  describe('reportLocalPatches', () => {
    ...
    test('reportLocalPatches shows /wtdd-reapply-patches for Copilot', () => {
      // Create patches directory with metadata
      const patchesDir = path.join(tmpDir, 'wtdd-local-patches');
      fs.mkdirSync(patchesDir, { recursive: true });
      fs.writeFileSync(path.join(patchesDir, 'backup-meta.json'), JSON.stringify({
        from_version: '1.0',
        files: ['skills/wtdd-test/SKILL.md']
      }));
      ...
      assert.ok(output.includes('/wtdd-reapply-patches'), 'uses dash format for Copilot');
      assert.ok(!output.includes('/wtdd:reapply-patches'), 'does not use colon format');
    });

    test('reportLocalPatches shows /wtdd-reapply-patches for Claude', () => {
      // Create patches directory with metadata
      const patchesDir = path.join(tmpDir, 'wtdd-local-patches');
      fs.mkdirSync(patchesDir, { recursive: true });
      fs.writeFileSync(path.join(patchesDir, 'backup-meta.json'), JSON.stringify({
        from_version: '1.0',
        files: ['what-the-dog-doing/bin/verify.cjs']
      }));
      ...
      assert.ok(result.length > 0, 'returns patched files list');
      const output = logs.join('\n');
      assert.ok(output.includes('/wtdd-reapply-patches'), 'uses hyphen format for Claude');
      assert.ok(!output.includes('/wtdd:reapply-patches'), 'does not use colon format for Claude');
    });
  });
});
...
const EXPECTED_SKILLS = fs.readdirSync(path.join(__dirname, '..', 'commands', 'wtdd'))
  .filter(f => f.endsWith('.md')).length;
const EXPECTED_AGENTS = fs.readdirSync(path.join(__dirname, '..', 'agents', 'wtdd'))
  .filter(f => f.startsWith('wtdd-') && f.endsWith('.md')).length;

function runCopilotInstall(cwd) {
  const env = { ...process.env };
  delete env.WTDD_TEST_MODE;
...
describe('E2E: Copilot full install verification', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-e2e-'));
  ...
  test('installs expected number of skill directories', () => {
    ...
    const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    const wtddSkills = entries.filter(e => e.isDirectory() && e.name.startsWith('wtdd-'));
    assert.strictEqual(wtddSkills.length, EXPECTED_SKILLS,
      `Expected ${EXPECTED_SKILLS} skill directories, got ${wtddSkills.length}`);
  });
  ...
  test('installs expected number of agent files', () => {
    ...
    const wtddAgents = files.filter(f => f.startsWith('wtdd-') && f.endsWith('.agent.md'));
    assert.strictEqual(wtddAgents.length, EXPECTED_AGENTS,
      `Expected ${EXPECTED_AGENTS} agent files, got ${wtddAgents.length}`);
  });

  test('installs all expected agent files', () => {
    const agentsDir = path.join(tmpDir, '.github', 'agents');
    const files = fs.readdirSync(agentsDir);
    const wtddAgents = files.filter(f => f.startsWith('wtdd-') && f.endsWith('.agent.md')).sort();
    const expected = [
      'wtdd-advisor-researcher.agent.md',
      'wtdd-assumptions-analyzer.agent.md',
      'wtdd-code-fixer.agent.md',
      'wtdd-code-reviewer.agent.md',
      'wtdd-codebase-mapper.agent.md',
      'wtdd-debugger.agent.md',
      'wtdd-doc-verifier.agent.md',
      'wtdd-doc-writer.agent.md',
      'wtdd-executor.agent.md',
      'wtdd-integration-checker.agent.md',
      'wtdd-intel-updater.agent.md',
      'wtdd-nyquist-auditor.agent.md',
      'wtdd-phase-researcher.agent.md',
      'wtdd-plan-checker.agent.md',
      'wtdd-planner.agent.md',
      'wtdd-project-researcher.agent.md',
      'wtdd-research-synthesizer.agent.md',
      'wtdd-roadmapper.agent.md',
      'wtdd-security-auditor.agent.md',
      'wtdd-ui-auditor.agent.md',
      'wtdd-ui-checker.agent.md',
      'wtdd-ui-researcher.agent.md',
      'wtdd-user-profiler.agent.md',
      'wtdd-verifier.agent.md',
    ].sort();
    assert.deepStrictEqual(wtddAgents, expected);
  });

  test('generates copilot-instructions.md with WTDD markers', () => {
    ...
    assert.ok(content.includes('<!-- WTDD Configuration'),
      'Should contain WTDD Configuration open marker');
    assert.ok(content.includes('<!-- /WTDD Configuration -->'),
      'Should contain WTDD Configuration close marker');
  });

  test('creates manifest with correct structure', () => {
    const manifestPath = path.join(tmpDir, '.github', 'wtdd-file-manifest.json');
    assert.ok(fs.existsSync(manifestPath), 'wtdd-file-manifest.json should exist');
  ...
  test('manifest contains expected file categories', () => {
    ...
    const engineEntries = keys.filter(k => k.startsWith('what-the-dog-doing/'));
    ...
    assert.ok(engineEntries.length > 0,
      'Should have what-the-dog-doing/ engine manifest entries');
  });
  ...
  test('engine directory contains required subdirectories and files', () => {
    const engineDir = path.join(tmpDir, '.github', 'what-the-dog-doing');
...
describe('E2E: Copilot uninstall verification', () => {
  ...
  test('removes engine directory', () => {
    ...
    assert.ok(!fs.existsSync(engineDir),
      'what-the-dog-doing directory should not exist after uninstall');
  });
  ...
  test('removes all WTDD skill directories', () => {
    const skillsDir = path.join(tmpDir, '.github', 'skills');
    if (fs.existsSync(skillsDir)) {
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      const wtddSkills = entries.filter(e => e.isDirectory() && e.name.startsWith('wtdd-'));
      assert.strictEqual(wtddSkills.length, 0,
        `Expected 0 WTDD skill directories after uninstall, found: ${wtddSkills.map(e => e.name).join(', ')}`);
    }
  });

  test('removes all WTDD agent files', () => {
    const agentsDir = path.join(tmpDir, '.github', 'agents');
    if (fs.existsSync(agentsDir)) {
      const files = fs.readdirSync(agentsDir);
      const wtddAgents = files.filter(f => f.startsWith('wtdd-') && f.endsWith('.agent.md'));
      assert.strictEqual(wtddAgents.length, 0,
        `Expected 0 WTDD agent files after uninstall, found: ${wtddAgents.join(', ')}`);
    }
  });

  describe('preserves non-WTDD content', () => {
    let td;

    beforeEach(() => {
      td = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-e2e-preserve-'));
    ...
    test('preserves non-WTDD content in skills directory', () => {
      // Add non-WTDD custom skill
      ...
      assert.ok(fs.existsSync(path.join(customSkillDir, 'SKILL.md')),
        'Non-WTDD skill directory and SKILL.md should be preserved after uninstall');
    });

    test('preserves non-WTDD content in agents directory', () => {
      // Add non-WTDD custom agent
      ...
      assert.ok(fs.existsSync(customAgentPath),
        'Non-WTDD agent file should be preserved after uninstall');
    });
  });
});
...
describe('Claude uninstall preserves user-generated files (#1423)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-preserve-'));
  ...
  test('preserves USER-PROFILE.md across uninstall', () => {
    const profilePath = path.join(tmpDir, '.claude', 'what-the-dog-doing', 'USER-PROFILE.md');
  ...
  test('preserves dev-preferences.md across uninstall', () => {
    const prefsDir = path.join(tmpDir, '.claude', 'commands', 'wtdd');
  ...
  test('still removes WTDD engine files during uninstall', () => {
    const profilePath = path.join(tmpDir, '.claude', 'what-the-dog-doing', 'USER-PROFILE.md');
    fs.writeFileSync(profilePath, '# Profile\n');

    // Verify engine files exist before uninstall
    const binDir = path.join(tmpDir, '.claude', 'what-the-dog-doing', 'bin');
  ...
  test('clean uninstall when no user files exist', () => {
    ...
    const wtddDir = path.join(tmpDir, '.claude', 'what-the-dog-doing');
    const cmdDir = path.join(tmpDir, '.claude', 'commands', 'wtdd');
    // Directories should be fully removed when no user files to preserve
    assert.ok(!fs.existsSync(wtddDir), 'what-the-dog-doing/ should not exist after clean uninstall');
    assert.ok(!fs.existsSync(cmdDir), 'commands/wtdd/ should not exist after clean uninstall');
  });
});
