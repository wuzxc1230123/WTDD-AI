 ...
 * (e.g., wtdd-intel-*.js) should NOT be flagged as stale by wtdd-check-update.js.
 *
 * The stale hooks scanner should only check hooks that are part of the current
 * distribution, not every wtdd-*.js file in the hooks directory.
 */
...
const CHECK_UPDATE_PATH = path.join(__dirname, '..', 'hooks', 'wtdd-check-update.js');
...
describe('orphaned hooks stale detection (#1750)', () => {
  test('stale hook scanner uses an allowlist of managed hooks, not a wildcard', () => {
    ...
    // The scanner MUST NOT use a broad `startsWith('wtdd-')` filter that catches
    // orphaned files from removed features (wtdd-intel-index.js, wtdd-intel-prune.js, etc.)
    ...
    // The child script must NOT have a broad wtdd-*.js wildcard filter
    const hasBroadFilter = /readdirSync\([^)]+\)\.filter\([^)]*startsWith\('wtdd-'\)\s*&&[^)]*endsWith\('\.js'\)/s.test(childScript);
    assert.ok(!hasBroadFilter,
      'scanner must NOT use broad startsWith("wtdd-") && endsWith(".js") filter — ' +
      'this catches orphaned hooks from removed features (e.g., wtdd-intel-index.js). ' +
