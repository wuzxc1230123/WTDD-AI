...
} = require('../what-the-dog-doing/bin/lib/security.cjs');
...
// Verify that wtdd-context-monitor and wtdd-statusline reject session_id values
...
describe('wtdd-context-monitor session_id path traversal', () => {
  const monitorPath = path.join(__dirname, '..', 'hooks', 'wtdd-context-monitor.js');
...
describe('wtdd-statusline session_id path traversal', () => {
  const statuslinePath = path.join(__dirname, '..', 'hooks', 'wtdd-statusline.js');
  const tmpDir = os.tmpdir();
  ...
  test('does not write bridge file for session_id with ../ traversal', () => {
    const maliciousId = '../../../etc/wtdd-test';
...
describe('validatePromptStructure', () => {
  ...
  test('accepts known valid tags in agent files', () => {
    const text = [
      ...
      '<available_agent_types>wtdd-executor</available_agent_types>',
    ].join('\n');
