/**
 * WTDD Quick Research Flag Tests
 *
 * Validates the --research flag for /wtdd-quick:
 ...
 * - Workflow spawns wtdd-phase-researcher for research
 */
...
describe('quick command: --research in frontmatter', () => {
  ...
  test('quick.md exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/wtdd/quick.md should exist');
  });
...
describe('quick workflow: research step', () => {
  ...
  test('research step spawns wtdd-phase-researcher', () => {
    content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.ok(
      researchSection.includes('subagent_type="wtdd-phase-researcher"'),
      'research step should spawn wtdd-phase-researcher agent'
    );
  });
