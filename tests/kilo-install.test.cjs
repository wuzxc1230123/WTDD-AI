/**
 * WTDD Tools Tests - Kilo Install Plumbing
 *
...
describe('Kilo config file helpers', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('wtdd-kilo-');
  });
...
describe('configureKiloPermissions', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('wtdd-kilo-perms-');
  ...
  test('writes WTDD permissions to kilo.json when config is missing', () => {
    configureKiloPermissions(true);

    const configPath = path.join(configDir, 'kilo.json');
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const wtddPath = `${configDir.replace(/\\/g, '/')}/what-the-dog-doing/*`;

    assert.strictEqual(config.permission.read[wtddPath], 'allow');
    assert.strictEqual(config.permission.external_directory[wtddPath], 'allow');
  });
  ...
  test('writes permissions to an explicit config dir argument', () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const wtddPath = `${explicitDir.replace(/\\/g, '/')}/what-the-dog-doing/*`;
...
describe('Source code integration (Kilo)', () => {
  ...
  const updateWorkflowSrc = fs.readFileSync(path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'update.md'), 'utf8');
  const reapplyPatchesSrc = fs.readFileSync(path.join(__dirname, '..', 'commands', 'wtdd', 'reapply-patches.md'), 'utf8');
