 ...
 *
 * Structural assertions for the /wtdd-import command and workflow files.
 */
...
const CMD_PATH = path.join(__dirname, '..', 'commands', 'wtdd', 'import.md');
const WF_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'import.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('import command file structure', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/wtdd/import.md should exist');
  });

  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'what-the-dog-doing/workflows/import.md should exist');
  });
});
...
describe('import command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*wtdd:import$/m);
  });
...
describe('import command references', () => {
  ...
  test('references the import workflow', () => {
    assert.ok(
      content.includes('@~/.claude/what-the-dog-doing/workflows/import.md'),
      'command should reference the workflow via @~/.claude/what-the-dog-doing/workflows/import.md'
    );
  });
});
...
describe('import workflow content', () => {
  ...
  test('includes plan-checker validation gate', () => {
    assert.ok(
      content.includes('wtdd-plan-checker'),
      'workflow should delegate validation to wtdd-plan-checker'
    );
  });

  test('no-args usage display is present', () => {
    assert.ok(
      content.includes('Usage: /wtdd-import'),
