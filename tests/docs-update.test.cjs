/**
 * WTDD Tools Tests - docs-update
 *
 * Integration tests for the docs-init wtdd-tools subcommand.
 * Covers: JSON output shape, project type detection, existing doc scanning,
 * WTDD marker detection, and doc tooling detection.
 *
...
describe('existing doc scanning', () => {
  ...
  test('detects WTDD marker in existing docs', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'README.md'),
      '<!-- generated-by: wtdd-doc-writer -->\n# README\n',
      'utf-8'
    );
    ...
    assert.strictEqual(readmeEntry.has_wtdd_marker, true, 'README.md should have WTDD marker');

    const notesEntry = data.existing_docs.find(d => d.path === 'NOTES.md');
    assert.ok(notesEntry, 'NOTES.md should appear in existing_docs');
    assert.strictEqual(notesEntry.has_wtdd_marker, false, 'NOTES.md should not have WTDD marker');
  });
});
