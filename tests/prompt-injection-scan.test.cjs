...
const { scanForInjection, INJECTION_PATTERNS } = require('../what-the-dog-doing/bin/lib/security.cjs');
...
const SCAN_DIRS = [
  'agents',
  'commands',
  'what-the-dog-doing/workflows',
  'what-the-dog-doing/bin/lib',
  'hooks',
];
...
const ALLOWLIST = new Set([
  'what-the-dog-doing/bin/lib/security.cjs',        // The security module itself
  'what-the-dog-doing/workflows/discuss-phase.md',  // Large workflow (~50K) with power mode + i18n
  'what-the-dog-doing/workflows/execute-phase.md',  // Large orchestration workflow (~51K) with wave execution + code-review gate
  'hooks/wtdd-prompt-guard.js',                  // The prompt guard hook
...
describe('codebase prompt injection scan', () => {
  ...
  test('no fake system boundary tags in non-allowlisted files', () => {
    ...
    for (const file of allFiles) {
      ...
      if (ALLOWLIST.has(relPath) || ALLOWLIST.has(normalizedRelPath)) continue;
