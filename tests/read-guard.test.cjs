/**
 * Tests for wtdd-read-guard.js PreToolUse hook.
 *
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'wtdd-read-guard.js');
...
describe('wtdd-read-guard hook', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempDir('wtdd-read-guard-');
  });
  ...
  test('hook is registered in build-hooks.js HOOKS_TO_COPY', () => {
    ...
    assert.ok(
      content.includes('wtdd-read-guard.js'),
      'wtdd-read-guard.js must be in HOOKS_TO_COPY so it ships in hooks/dist/'
    );
  });
  ...
  test('hook is registered in install.js uninstall hook list', () => {
    ...
    assert.ok(
      content.includes("'wtdd-read-guard.js'"),
      'wtdd-read-guard.js must be in the uninstall wtddHooks list'
    );
  });
