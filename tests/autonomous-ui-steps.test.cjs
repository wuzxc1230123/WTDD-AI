...
const WORKFLOW_PATH = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'autonomous.md');
...
describe('autonomous workflow ui-phase and ui-review integration (#1375)', () => {
  ...
  describe('step 3a.5 — UI design contract before planning', () => {
    ...
    test('UI design contract step invokes wtdd:ui-phase skill', () => {
      assert.ok(
        content.includes('skill="wtdd-ui-phase"'),
        'should invoke wtdd-ui-phase via Skill()'
      );
    });
  ...
  describe('step 3d.5 — UI review after execution', () => {
    ...
    test('UI review step invokes wtdd:ui-review skill', () => {
      assert.ok(
        content.includes('skill="wtdd-ui-review"'),
        'should invoke wtdd-ui-review via Skill()'
      );
    });
