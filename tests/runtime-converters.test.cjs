...
// Sample Claude agent frontmatter (matches actual WTDD agent format)
const SAMPLE_AGENT = `---
name: wtdd-executor
...
for (const { label, convert, configDir } of flatRuntimeSuites) {
  describe(`${label} agent conversion (isAgent: true)`, () => {
    test('keeps name: field for agents', () => {
      ...
      assert.ok(frontmatter.includes('name: wtdd-executor'), 'name: should be preserved for agents');
    });
    ...
    test('strips skills: array', () => {
      ...
      assert.ok(!frontmatter.includes('wtdd-executor-workflow'), 'skill entries should be stripped');
    });
    ...
    test('keeps description: field', () => {
      const result = convert(SAMPLE_AGENT, { isAgent: true });
      const frontmatter = result.split('---')[1];
      assert.ok(frontmatter.includes('description: Executes WTDD plans'), 'description should be kept');
    });

    test('preserves body content', () => {
      const result = convert(SAMPLE_AGENT, { isAgent: true });
      assert.ok(result.includes('<role>'), 'body should be preserved');
      assert.ok(result.includes('You are a WTDD plan executor.'), 'body content should be intact');
    });
...
Use ./.claude/hooks/wtdd-statusline.js during local testing.
...
Use \${PHASE} in shell examples.
    ...
    assert.ok(frontmatter.includes('name: wtdd-codebase-mapper'), 'keeps name');
    ...
    assert.ok(!frontmatter.includes('wtdd-mapper-workflow'), 'drops skills list items');
