...
const WTDD_ROOT = path.join(__dirname, '..', 'what-the-dog-doing');
const UI_BRAND = path.join(WTDD_ROOT, 'references', 'ui-brand.md');
const CONTINUATION_FORMAT = path.join(WTDD_ROOT, 'references', 'continuation-format.md');
const WORKFLOWS_DIR = path.join(WTDD_ROOT, 'workflows');
...
describe('workflow files — no <sub>/clear patterns in Next Up blocks', () => {
  ...
  test('no workflow file contains <sub> with /clear', () => {
    ...
    for (const filePath of workflowFiles) {
      ...
      if (matches) {
        failures.push({
          file: path.relative(WTDD_ROOT, filePath),
...
describe('reference files — no <sub>/clear patterns', () => {
  const referencesDir = path.join(WTDD_ROOT, 'references');
