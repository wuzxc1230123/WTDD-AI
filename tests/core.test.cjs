...
const {
  ...
  detectSubRepos,
  planningDir,
} = require('../what-the-dog-doing/bin/lib/core.cjs');
...
describe('loadConfig', () => {
  ...
  test('returns model_overrides when present (REG-01)', () => {
    writeConfig({ model_overrides: { 'wtdd-executor': 'opus' } });
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'wtdd-executor': 'opus' });
  });
  ...
  test('known config keys are derived from VALID_CONFIG_KEYS (not hardcoded)', () => {
    ...
    const { VALID_CONFIG_KEYS } = require('../what-the-dog-doing/bin/lib/config.cjs');
...
describe('loadConfig commit_docs gitignore auto-detection (#1250)', () => {
  ...
  test('commit_docs auto-detect works with no config.json', () => {
    ...
    // a project without config.json hasn't been initialized by WTDD yet.
...
describe('resolveModelInternal', () => {
  ...
  describe('model profile structural validation', () => {
    test('all known agents resolve to a valid string for each profile', () => {
      const knownAgents = ['wtdd-planner', 'wtdd-executor', 'wtdd-phase-researcher', 'wtdd-codebase-mapper'];
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      writeConfig({
        ...
        model_overrides: { 'wtdd-executor': 'haiku' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-executor'), 'haiku');
    });

    test('opus override resolves to opus', () => {
      writeConfig({
        model_overrides: { 'wtdd-executor': 'opus' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-executor'), 'opus');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      // wtdd-planner not overridden, should use quality profile -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-planner'), 'opus');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-nonexistent'), 'sonnet');
    });
    ...
    test('defaults to balanced profile when model_profile missing', () => {
      writeConfig({});
      // balanced profile, wtdd-planner -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-planner'), 'opus');
    });
  });
  ...
  describe('resolve_model_ids: "omit"', () => {
    test('returns empty string for known agents', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-planner'), '');
    });

    test('returns empty string for unknown agents', () => {
      writeConfig({ resolve_model_ids: 'omit' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-nonexistent'), '');
    });

    test('still respects model_overrides even when omit', () => {
      writeConfig({
        resolve_model_ids: 'omit',
        model_overrides: { 'wtdd-planner': 'openai/gpt-5.4' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'wtdd-planner'), 'openai/gpt-5.4');
    });
...
describe('safeReadFile', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-core-test-'));
  });
...
describe('stale hook filter', () => {
  test('filter should only match wtdd-prefixed .js files', () => {
    const files = [
      ...
      'wtdd-check-update.js.bak',         // backup file
      'README.md',                       // non-js file
    ];

    const wtddFilter = f => f.startsWith('wtdd-') && f.endsWith('.js');
    const filtered = files.filter(wtddFilter);
    ...
    assert.deepStrictEqual(filtered, [
      ...
      'wtdd-statusline.js',
      'wtdd-workflow-guard.js',
    ], 'should only include wtdd-prefixed .js files');

    assert.ok(!filtered.includes('guard-edits-outside-project.js'), 'must not include user hooks');
    assert.ok(!filtered.includes('my-custom-hook.js'), 'must not include non-wtdd hooks');
  });
});
...
describe('stale hook path', () => {
  test('wtdd-check-update.js checks configDir/hooks/ where hooks are actually installed (#1421)', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'wtdd-check-update.js'), 'utf-8'
    );
    // Hooks are installed at configDir/hooks/ (e.g. ~/.claude/hooks/),
    // not configDir/what-the-dog-doing/hooks/ which doesn't exist (#1421)
    assert.ok(
...
describe('shared cache directory (#1421)', () => {
  test('wtdd-check-update.js writes cache to shared ~/.cache/wtdd/ directory', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'wtdd-check-update.js'), 'utf-8'
    );
    ...
    assert.ok(
      content.includes("path.join(homeDir, '.cache', 'wtdd')"),
      'check-update must write cache to ~/.cache/wtdd/ (shared, tool-agnostic)'
    );
  });

  test('wtdd-statusline.js checks shared cache first, falls back to legacy (#1421)', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'wtdd-statusline.js'), 'utf-8'
    );
    // Statusline must check the shared cache path first
    assert.ok(
      content.includes("path.join(homeDir, '.cache', 'wtdd', 'wtdd-update-check.json')"),
      'statusline must check shared cache at ~/.cache/wtdd/wtdd-update-check.json'
    );
    // Must fall back to legacy runtime-specific cache for backward compat
    assert.ok(
      content.includes("path.join(claudeDir, 'cache', 'wtdd-update-check.json')"),
      'statusline must fall back to legacy cache at claudeDir/cache/wtdd-update-check.json'
    );
...
describe('resolveWorktreeRoot', () => {
  const { resolveWorktreeRoot } = require('../what-the-dog-doing/bin/lib/core.cjs');
  let tmpDir;
...
describe('resolveWorktreeRoot with linked worktree .planning/', () => {
  ...
  function initBareGitRepo() {
    const dir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-wt-main-')));
  ...
  test('returns linked worktree cwd when it has its own .planning/', () => {
    ...
    worktreeDir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-wt-linked-')));
...
describe('monorepo worktree CWD preservation', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-monorepo-wt-'));
  });
...
describe('withPlanningLock', () => {
  const { withPlanningLock, planningDir } = require('../what-the-dog-doing/bin/lib/core.cjs');
  let tmpDir;
...
describe('detectSubRepos', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-detect-test-'));
  });
...
describe('loadConfig sub_repos auto-sync', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-sync-test-'));
...
describe('findProjectRoot', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-root-test-'));
  });
...
describe('reapStaleTempFiles', () => {
  test('removes stale wtdd-*.json files older than maxAgeMs', () => {
    const tmpDir = os.tmpdir();
    const stalePath = path.join(tmpDir, `wtdd-reap-test-${Date.now()}.json`);
    fs.writeFileSync(stalePath, '{}');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(stalePath, oldTime, oldTime);

    reapStaleTempFiles('wtdd-reap-test-', { maxAgeMs: 5 * 60 * 1000 });

    assert.ok(!fs.existsSync(stalePath), 'stale file should be removed');
  });

  test('preserves fresh wtdd-*.json files', () => {
    const tmpDir = os.tmpdir();
    const freshPath = path.join(tmpDir, `wtdd-reap-fresh-${Date.now()}.json`);
    fs.writeFileSync(freshPath, '{}');

    reapStaleTempFiles('wtdd-reap-fresh-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('removes stale temp directories when present', () => {
    const tmpDir = os.tmpdir();
    const staleDir = fs.mkdtempSync(path.join(tmpDir, 'wtdd-reap-dir-'));
    fs.writeFileSync(path.join(staleDir, 'data.jsonl'), 'test');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(staleDir, oldTime, oldTime);

    reapStaleTempFiles('wtdd-reap-dir-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('does not throw on empty or missing prefix matches', () => {
    assert.doesNotThrow(() => {
      reapStaleTempFiles('wtdd-nonexistent-prefix-xyz-', { maxAgeMs: 0 });
    });
  });
});
...
describe('planningDir', () => {
  ...
  beforeEach(() => {
    savedProject = process.env.WTDD_PROJECT;
    savedWorkstream = process.env.WTDD_WORKSTREAM;
    delete process.env.WTDD_PROJECT;
    delete process.env.WTDD_WORKSTREAM;
  });

  afterEach(() => {
    if (savedProject !== undefined) process.env.WTDD_PROJECT = savedProject;
    else delete process.env.WTDD_PROJECT;
    if (savedWorkstream !== undefined) process.env.WTDD_WORKSTREAM = savedWorkstream;
    else delete process.env.WTDD_WORKSTREAM;
  });
  ...
  test('reads WTDD_PROJECT from env when project param is undefined', () => {
    process.env.WTDD_PROJECT = 'env-project';
