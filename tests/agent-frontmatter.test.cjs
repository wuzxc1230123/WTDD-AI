...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        const hasWorkaround = content.includes('First, read ~/.claude/agents/wtdd-');
        assert.ok(
          !hasWorkaround,
  ...
  test('diagnose-issues uses wtdd-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="wtdd-debugger"'),
      'diagnose-issues should spawn wtdd-debugger, not general-purpose'
    );
  });
  ...
  test('workflows spawning named agents have <available_agent_types> listing (#1357)', () => {
    ...
    // workflows that spawn named WTDD agents.
    const dirs = [WORKFLOWS_DIR, COMMANDS_DIR];
    for (const dir of dirs) {
      if (!fs.existsSync(dir)) continue;
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.md'));
...
describe('CLAUDEMD: CLAUDE.md compliance enforcement', () => {
  test('wtdd-plan-checker has Dimension 10: CLAUDE.md Compliance', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-plan-checker.md'), 'utf-8');
    assert.ok(
      content.includes('Dimension 10: CLAUDE.md Compliance'),
      'wtdd-plan-checker must have Dimension 10 for CLAUDE.md compliance checking'
    );
    assert.ok(
      content.includes('claude_md_compliance'),
      'wtdd-plan-checker must use claude_md_compliance as dimension identifier'
    );
  });

  test('wtdd-phase-researcher has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'wtdd-phase-researcher must enforce CLAUDE.md directives during research'
    );
    assert.ok(
      content.includes('Project Constraints (from CLAUDE.md)'),
      'wtdd-phase-researcher must output a Project Constraints section from CLAUDE.md'
    );
  });

  test('wtdd-executor has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-executor.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'wtdd-executor must enforce CLAUDE.md directives during execution'
    );
    assert.ok(
      content.includes('CLAUDE.md rule — it takes precedence over plan instructions'),
      'wtdd-executor must specify CLAUDE.md precedence over plan instructions'
    );
  });

  test('all three agents read CLAUDE.md in project_context', () => {
    const agents = ['wtdd-plan-checker', 'wtdd-phase-researcher', 'wtdd-executor'];
...
describe('VERIFY: data-flow trace, environment audit, and behavioral spot-checks', () => {
  test('wtdd-verifier has Step 4b: Data-Flow Trace', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 4b: Data-Flow Trace'),
      'wtdd-verifier must have Step 4b for data-flow tracing'
    );
    assert.ok(
      content.includes('HOLLOW'),
      'wtdd-verifier must define HOLLOW status for wired-but-disconnected artifacts'
    );
    assert.ok(
      content.includes('DISCONNECTED'),
      'wtdd-verifier must define DISCONNECTED status for missing data sources'
    );
  });

  test('wtdd-verifier has Step 7b: Behavioral Spot-Checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 7b: Behavioral Spot-Checks'),
      'wtdd-verifier must have Step 7b for behavioral spot-checks'
    );
    assert.ok(
      content.includes('SKIP'),
      'wtdd-verifier spot-checks must support SKIP status for untestable items'
    );
  });

  test('wtdd-verifier VERIFICATION.md template includes data-flow and spot-check sections', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-verifier.md'), 'utf-8');
    assert.ok(
  ...
  test('wtdd-verifier success criteria include data-flow and spot-checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Data-flow trace (Level 4)'),
  ...
  test('wtdd-phase-researcher has Step 2.6: Environment Availability Audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('Step 2.6: Environment Availability Audit'),
      'wtdd-phase-researcher must have Step 2.6 for environment availability auditing'
    );
    assert.ok(
      content.includes('Environment Availability'),
      'wtdd-phase-researcher must include Environment Availability section in RESEARCH.md template'
    );
  });

  test('wtdd-phase-researcher success criteria include environment audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'wtdd-phase-researcher.md'), 'utf-8');
    assert.ok(
...
describe('DISCUSS: discussion log generation', () => {
  ...
  test('discussion-log template exists', () => {
    const templatePath = path.join(__dirname, '..', 'what-the-dog-doing', 'templates', 'discussion-log.md');
    assert.ok(
...
describe('COMPAT: agents must not use runtime-specific frontmatter keys', () => {
  ...
  const AGENTS_WITH_WRITE = ['wtdd-executor', 'wtdd-debugger'];
