/**
 * Regression guard for #1767: wtdd-workflow-guard.js must be registered in settings.json
 *
 ...
 *
 * Also tests the broader anti-pattern: every hook in wtddHooks that is a JS
...
describe('workflow-guard hook registration (#1767)', () => {
  test('install.js constructs a command path variable for wtdd-workflow-guard.js', () => {
    ...
    const commandConstructionLines = lines.filter(line =>
      line.includes('wtdd-workflow-guard.js') &&
      (line.includes('buildHookCommand') || line.includes("'node '"))
    );
    assert.ok(
      commandConstructionLines.length > 0,
      [
        'install.js must construct a command path for wtdd-workflow-guard.js',
        '(e.g. buildHookCommand or node + dirName pattern).',
        'Currently only referenced in wtddHooks cleanup array.',
      ].join(' ')
    );
  });
...
describe('hook registration completeness anti-pattern guard', () => {
  test('every JS hook in wtddHooks has a command construction in install.js', () => {
    const content = fs.readFileSync(INSTALL_JS, 'utf-8');
    // Extract wtddHooks array entries
    const hooksMatch = content.match(/wtddHooks\s*=\s*\[([^\]]+)\]/);
    assert.ok(hooksMatch, 'wtddHooks array must exist in install.js');
    ...
    for (const hook of jsHooks) {
      ...
      // that references the hook filename (not just the wtddHooks array or uninstall filter)
    ...
    assert.strictEqual(
      missing.length, 0,
      [
        'Every JS hook in wtddHooks must have a command construction in install.js.',
