 ...
 * the YAML frontmatter override format, and is referenced by wtdd-verifier.md.
 */
...
describe('verification overrides reference (#1747)', () => {
  ...
  describe('what-the-dog-doing/references/verification-overrides.md', () => {
    const refPath = path.join(ROOT, 'what-the-dog-doing', 'references', 'verification-overrides.md');
    let content;
    ...
    test('documents milestone audit surfacing', () => {
      content = content || fs.readFileSync(refPath, 'utf-8');
      assert.ok(
        content.includes('wtdd-audit-milestone') || content.includes('audit-milestone') || content.includes('milestone'),
  ...
  describe('agents/wtdd-verifier.md references overrides', () => {
    const verifierPath = path.join(ROOT, 'agents', 'wtdd', 'wtdd-verifier.md');
    let verifierContent;

    test('wtdd-verifier.md exists', () => {
      assert.ok(fs.existsSync(verifierPath), 'wtdd-verifier.md should exist');
    ...
    test('references verification-overrides.md in required_reading', () => {
      ...
      assert.ok(
        ...
        'wtdd-verifier.md should reference verification-overrides.md'
      );
    });
    ...
    test('verifier includes Step 3b for override check before FAIL', () => {
      ...
      assert.ok(
        ...
        'wtdd-verifier.md should include a Step 3b override check'
      );
    });
    ...
    test('verifier frontmatter template includes overrides_applied', () => {
      ...
      assert.ok(
        ...
        'wtdd-verifier.md frontmatter template should include overrides_applied counter'
      );
    });
  });
});
