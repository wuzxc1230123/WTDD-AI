 ...
 *
 * Regression tests for: https://github.com/wtdd-build/what-the-dog-doing/issues/1343
 */
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling wtdd-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls wtdd-tools.cjs init
      if (/INIT=\$\(node.*wtdd-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling wtdd-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
