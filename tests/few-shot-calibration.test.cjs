...
const REFS_DIR = path.join(__dirname, '..', 'what-the-dog-doing', 'references', 'few-shot-examples');
...
describe('few-shot calibration examples', () => {
  ...
  describe('agent files reference few-shot examples', () => {
    test('wtdd-plan-checker.md contains reference to plan-checker few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'wtdd-plan-checker.md'));
      assert.match(content, /@~\/\.claude\/what-the-dog-doing\/references\/few-shot-examples\/plan-checker\.md/);
    });

    test('wtdd-verifier.md contains reference to verifier few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'wtdd-verifier.md'));
      assert.match(content, /@~\/\.claude\/what-the-dog-doing\/references\/few-shot-examples\/verifier\.md/);
    });
  });
