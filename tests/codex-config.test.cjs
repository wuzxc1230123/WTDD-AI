/**
 * WTDD Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('wtdd-execute-phase');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('wtdd-plan-phase');
    assert.ok(result.includes('`$wtdd-plan-phase`'), 'has $skillName invocation');
    assert.ok(result.includes('{{WTDD_ARGS}}'), 'has WTDD_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('wtdd-discuss-phase');
...
You are a WTDD plan executor.
    ...
    assert.ok(result.includes('"wtdd-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes WTDD plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: wtdd-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes WTDD plans with atomic commits'), 'purpose from description');
...
Run /wtdd:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$wtdd-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/wtdd:execute-phase'), 'original slash command removed');
  });
...
INIT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" state load)
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: resolve"`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$HOME/.codex/what-the-dog-doing/bin/wtdd-tools.cjs'), 'replaces $HOME/.claude/ with $HOME/.codex/');
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('wtdd-executor', sampleAgent);
    assert.ok(result.includes('name = "wtdd-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('wtdd-unknown', minimalAgent);
    assert.ok(result.includes('name = "wtdd-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "WTDD agent wtdd-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('wtdd-unknown', sampleAgent);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'wtdd-executor', 'wtdd-planner', 'wtdd-phase-researcher',
      'wtdd-project-researcher', 'wtdd-research-synthesizer', 'wtdd-verifier',
      'wtdd-codebase-mapper', 'wtdd-roadmapper', 'wtdd-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['wtdd-plan-checker', 'wtdd-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'wtdd-executor', description: 'Executes plans' },
    { name: 'wtdd-planner', description: 'Creates plans' },
  ];

  test('starts with WTDD marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(WTDD_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('does not include feature flags or agents table header', () => {
    ...
    // Should not have bare [agents] table header (only [agents.wtdd-*] sections)
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.wtdd-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.wtdd-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/wtdd-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/wtdd-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/wtdd-planner.toml"'), 'absolute planner path');
...
describe('stripGsdFromCodexConfig', () => {
  test('returns null for WTDD-only config', () => {
    const content = `${WTDD_CODEX_MARKER}\n[features]\nmulti_agent = true\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.strictEqual(result, null, 'returns null when WTDD-only');
  });
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes('multi_agent'), 'removes WTDD content');
    assert.ok(!result.includes(WTDD_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(WTDD_CODEX_MARKER), 'strips marker');
  });

  test('removes [agents.wtdd-*] sections', () => {
    const content = `[agents.wtdd-executor]\ndescription = "test"\nconfig_file = "agents/wtdd-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!result.includes('[agents.wtdd-executor]'), 'removes WTDD agent section');
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(WTDD_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('[agents.wtdd-executor]'), 'has agent');
    assert.ok(!content.includes('[features]'), 'no features section');
    assert.ok(!content.includes('multi_agent'), 'no multi_agent');
  });

  test('case 2: replaces existing WTDD block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'wtdd-executor', description: 'Updated description' },
      { name: 'wtdd-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.wtdd-planner]'), 'has new agent');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(WTDD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without WTDD marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(WTDD_CODEX_MARKER), 'adds marker');
    assert.ok(content.includes('[agents.wtdd-executor]'), 'has agent');
  });

  test('case 3 with existing [features]: preserves user features, does not inject WTDD keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    assert.ok(content.includes(WTDD_CODEX_MARKER), 'adds marker for agents block');
    assert.ok(content.includes('[agents.wtdd-executor]'), 'has agent');
  });

  test('case 3 strips existing [agents.wtdd-*] sections before appending fresh block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const existing = [
      '[model]',
      'name = "o3"',
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    const wtddAgentCount = (content.match(/^\[agents\.wtdd-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(WTDD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-WTDD agent section');
    assert.strictEqual(wtddAgentCount, 1, 'keeps exactly one WTDD agent section');
    assert.strictEqual(markerCount, 1, 'adds exactly one marker block');
    assert.ok(!/\n{3,}# WTDD Agent Configuration/.test(content), 'does not leave extra blank lines before marker block');
  });
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.wtdd-executor]'), 'has agent from fresh block');
  });

  test('case 2 strips leaked [agents] and [agents.wtdd-*] from before content', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
    ...
    assert.ok(!beforeMarker.includes('[agents.wtdd-'), 'no leaked [agents.wtdd-*] above marker');
  });

  test('case 2 strips leaked WTDD-managed sections above marker in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.wtdd-executor\]\s*$/gm), 0, 'removes leaked WTDD agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.wtdd-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked WTDD sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.wtdd-executor\]\s*$/gm), 0, 'removes leaked WTDD agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.wtdd-executor\]\s*$/gm), 1, 'keeps one managed agent section in the WTDD block');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending WTDD block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
    ...
    assert.ok(content.includes(`${WTDD_CODEX_MARKER}\r\n`), 'writes marker with CRLF');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('uses the first newline style when appending WTDD block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes(WTDD_CODEX_MARKER), 'has WTDD marker');
    assert.ok(config.includes('[agents.wtdd-executor]'), 'has executor agent');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'wtdd-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'wtdd-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'wtdd-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "wtdd-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes WTDD plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'wtdd-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "wtdd-plan-checker"'), 'plan-checker has name');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-codex-e2e-'));
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# WTDD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes WTDD SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'writes one WTDD update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'adds one WTDD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. WTDD must not prepend [features] before
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.wtdd-codebase-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'does not duplicate WTDD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'keeps one WTDD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'remains idempotent for the WTDD hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'adds one WTDD update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 0, 'does not add the WTDD hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.wtdd-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'adds one WTDD update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-WTDD hooks', () => {
    writeCodexConfig(codexHome, [
      '[features]',
      'codex_hooks = true',
      'other_feature = true',
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-WTDD hook exactly once');
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'keeps one WTDD update hook');
    assertUsesOnlyEol(content, '\r\n');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'adds the WTDD update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# WTDD Agent Configuration — managed by what-the-dog-doing installer\n`), 'writes the managed agent block using the first newline style');
    assert.ok(content.includes('# WTDD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the WTDD hook block using the first newline style');
    ...
    assert.strictEqual(countMatches(content, /wtdd-check-update\.js/g), 1, 'does not duplicate the WTDD hook block');
    assertNoDraftRootKeys(content);
  });
});
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'wtdd-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('fresh install removes the WTDD-added codex_hooks feature on uninstall', () => {
    runCodexInstall(codexHome);

    const cleaned = stripGsdFromCodexConfig(readCodexConfig(codexHome));
    assert.strictEqual(cleaned, null, 'fresh WTDD-only config strips back to nothing');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^codex_hooks = true$/gm), 0, 'removes the WTDD-added codex_hooks key');
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-WTDD hooks');
    assert.strictEqual(countMatches(cleaned, /wtdd-check-update\.js/g), 0, 'removes only the WTDD update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.wtdd-/g), 0, 'removes managed WTDD agent sections');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^features\.codex_hooks = true$/gm), 0, 'removes the dotted WTDD codex_hooks key');
    assert.strictEqual(countMatches(cleaned, /^\[features\]\s*$/gm), 0, 'does not leave behind a [features] table');
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-WTDD hooks');
    assert.strictEqual(countMatches(cleaned, /wtdd-check-update\.js/g), 0, 'removes the WTDD update hook');
  });
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing WTDD hook state', () => {
