...
describe('generate-claude-md', () => {
  ...
  test('creates CLAUDE.md with workflow enforcement section', () => {
    ...
    assert.ok(content.includes('## WTDD Workflow Enforcement'));
    assert.ok(content.includes('/wtdd-quick'));
    assert.ok(content.includes('/wtdd-debug'));
    assert.ok(content.includes('/wtdd-execute-phase'));
    assert.ok(content.includes('Do not make direct repo edits outside a WTDD workflow'));
  });
...
describe('new-project workflow includes CLAUDE.md generation', () => {
  const workflowPath = path.join(__dirname, '..', 'what-the-dog-doing', 'workflows', 'new-project.md');
...
describe('generate-claude-md skills section', () => {
  ...
  test('includes skills fallback when no skills directories exist', () => {
    ...
    const content = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    assert.ok(content.includes('<!-- WTDD:skills-start'));
    assert.ok(content.includes('<!-- WTDD:skills-end -->'));
  ...
  test('skips wtdd- prefixed skill directories', () => {
    const wtddSkillDir = path.join(tmpDir, '.claude', 'skills', 'wtdd-plan-phase');
    const userSkillDir = path.join(tmpDir, '.claude', 'skills', 'my-feature');
    fs.mkdirSync(wtddSkillDir, { recursive: true });
    fs.mkdirSync(userSkillDir, { recursive: true });
    fs.writeFileSync(
      path.join(wtddSkillDir, 'SKILL.md'),
      '---\nname: wtdd-plan-phase\ndescription: WTDD internal skill.\n---\n'
    );
    fs.writeFileSync(
    ...
    const content = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    assert.ok(!content.includes('wtdd-plan-phase'));
  ...
  test('skills section appears between architecture and workflow', () => {
    ...
    const workflowIdx = content.indexOf('## WTDD Workflow Enforcement');
