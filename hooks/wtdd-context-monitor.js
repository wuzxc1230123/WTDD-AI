...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Detect if WTDD is active (has .planning/STATE.md in working directory)
    ...
    if (isCritical) {
      message = isGsdActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          ...
          'WTDD state is already tracked in STATE.md. Inform the user so they can run ' +
          '/wtdd-pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
