#!/usr/bin/env node
// wtdd-hook-version: {{WTDD_VERSION}}
// Check for WTDD updates in background, write result to cache
...
function detectConfigDir(baseDir) {
  ...
  if (envDir && fs.existsSync(path.join(envDir, 'what-the-dog-doing', 'VERSION'))) {
    return envDir;
  }
  for (const dir of ['.claude', '.gemini', '.config/kilo', '.kilo', '.config/opencode', '.opencode']) {
    if (fs.existsSync(path.join(baseDir, dir, 'what-the-dog-doing', 'VERSION'))) {
...
const cacheDir = path.join(homeDir, '.cache', 'wtdd');
const cacheFile = path.join(cacheDir, 'wtdd-update-check.json');

// VERSION file locations (check project first, then global)
const projectVersionFile = path.join(projectConfigDir, 'what-the-dog-doing', 'VERSION');
const globalVersionFile = path.join(globalConfigDir, 'what-the-dog-doing', 'VERSION');
...
const child = spawn(process.execPath, ['-e', `
  ...
  // Only check hooks that WTDD currently ships — orphaned files from removed features
  // (e.g., wtdd-intel-*.js) must be ignored to avoid permanent stale warnings (#1750)
  const MANAGED_HOOKS = [
      ...
      if (fs.existsSync(hooksDir)) {
        ...
        for (const hookFile of hookFiles) {
          try {
            ...
            const versionMatch = content.match(/\\/\\/ wtdd-hook-version:\\s*(.+)/);
            if (versionMatch) {
  ...
  let latest = null;
  try {
    latest = execSync('npm view what-the-dog-doing-cc version', { encoding: 'utf8', timeout: 10000, windowsHide: true }).trim();
  } catch (e) {}
