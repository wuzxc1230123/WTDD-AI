...
- `--path` — Target directory. Defaults to `~/wtdd-workspaces/<name>`
...
<objective>
Create a physical workspace directory containing copies of specified git repos (as worktrees or clones) with an independent `.planning/` directory for isolated WTDD sessions.

**Use cases:**
- Multi-repo orchestration: work on a subset of repos in parallel with isolated WTDD state
...
**After this command:** `cd` into the workspace and run `/wtdd-new-project` to initialize WTDD.
</objective>

<execution_context>
@~/.claude/what-the-dog-doing/workflows/new-workspace.md
@~/.claude/what-the-dog-doing/references/ui-brand.md
</execution_context>

<process>
Execute the new-workspace workflow from @~/.claude/what-the-dog-doing/workflows/new-workspace.md end-to-end.
