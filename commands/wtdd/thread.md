...
```
No threads found. Create one with: /wtdd-thread <description>
```
</mode_list>
...
5. Commit:
   ```bash
   node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Resume anytime with: /wtdd-thread {slug}
   ```
</mode_create>
...
- Lighter weight than /wtdd-pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /wtdd-add-phase or /wtdd-add-backlog with context from the thread
- Thread files live in .planning/threads/ — no collision with phases or other WTDD structures
</notes>
