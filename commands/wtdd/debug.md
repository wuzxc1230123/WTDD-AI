...
**Orchestrator role:** Gather symptoms, spawn wtdd-debugger agent, handle checkpoints, spawn continuations.
...
```bash
debugger_model=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" resolve-model wtdd-debugger --raw)
```
...
- Offer options:
  ...
  - "Plan fix" — suggest `/wtdd-plan-phase --gaps`
  - "Manual fix" — done

**If `## DEBUG COMPLETE` (find_and_fix mode):**
- Display root cause and fix summary
- Offer options:
  - "Plan fix" — suggest `/wtdd-plan-phase --gaps` if further work needed
  - "Done" — mark resolved
...
- [ ] wtdd-debugger spawned with context
