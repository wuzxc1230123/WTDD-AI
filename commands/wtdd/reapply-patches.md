---
name: wtdd:reapply-patches
description: Reapply local modifications after a WTDD update
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, AskUserQuestion
---

<purpose>
After a WTDD update wipes and reinstalls files, this command merges user's previously saved local modifications back into the new version. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to reliably distinguish user customizations from version drift.

**Critical invariant:** Every file in `wtdd-local-patches/` was backed up because the installer's hash comparison detected it was modified. The workflow must NEVER conclude "no custom content" for any backed-up file — that is a logical contradiction. When in doubt, classify as CONFLICT requiring user review, not SKIP.
</purpose>
...
if [ -n "$KILO_CONFIG_DIR" ]; then
  candidate="$(expand_home "$KILO_CONFIG_DIR")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$KILO_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/wtdd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/wtdd-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/wtdd-local-patches"
  elif [ -d "$HOME/.config/opencode/wtdd-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/wtdd-local-patches"
  elif [ -d "$HOME/.opencode/wtdd-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/wtdd-local-patches"
  elif [ -d "$HOME/.gemini/wtdd-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/wtdd-local-patches"
  elif [ -d "$HOME/.codex/wtdd-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/wtdd-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/wtdd-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/wtdd-local-patches" ]; then
      PATCHES_DIR="./$dir/wtdd-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /wtdd-update
after modifying any WTDD workflow, command, or agent files.
```
Exit.

## Step 2: Determine baseline for three-way comparison

The quality of the merge depends on having a **pristine baseline** — the original unmodified version of each file from the pre-update WTDD release. This enables three-way comparison:
- **Pristine baseline** (original WTDD file before any user edits)
- **User's version** (backed up in `wtdd-local-patches/`)
...
```
When `HAS_GIT=true`, use `git log` to find the commit where WTDD was originally installed (before user edits). For each file, the pristine baseline can be extracted with:
```bash
...
Check if a `wtdd-pristine/` directory exists alongside `wtdd-local-patches/`:
```bash
PRISTINE_DIR="$CONFIG_DIR/wtdd-pristine"
```
...
1. **Read the backed-up version** (user's modified copy from `wtdd-local-patches/`)
2. **Read the newly installed version** (current file after update)
3. **If available, read the pristine baseline** (from git history or `wtdd-pristine/`)
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `$HOME/.claude/`), variable additions (`${WTDD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
...
git -C "$CONFIG_DIR" log --oneline --no-merges -- "{file_path}" | grep -v "wtdd:update\|WTDD update\|wtdd-install"
```
...
Ask user:
- "Keep patch backups for reference?" → preserve `wtdd-local-patches/`
- "Clean up patch backups?" → remove `wtdd-local-patches/` directory
...
- [ ] Three-way merge used when pristine baseline available (git history or wtdd-pristine/)
