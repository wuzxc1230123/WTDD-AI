...
**DO NOT use the wtdd-tools config get-value command** -- it hard-exits on missing keys.
...
Intel system is disabled. To activate:
  ...
  node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs config-set intel.enabled true

Then run /wtdd-intel refresh to build the initial index.
```
...
```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs intel query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If no matches found, display: `No intel matches for '<term>'. Try /wtdd-intel refresh to build the index.`
...
```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs intel status
```
...
```bash
node $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs intel diff
```
...
```
WTDD > Spawning intel-updater agent to analyze codebase...
```
...
Task(
  ...
  prompt="You are the wtdd-intel-updater agent. Your job is to analyze this codebase and write/update intelligence files in .planning/intel/.

Project root: ${CWD}
wtdd-tools path: $HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs
...
4. Use wtdd-tools intel extract-exports <file> to analyze source files
5. Use wtdd-tools intel patch-meta <file> to update timestamps after writing
6. Use wtdd-tools intel validate to check your output
...
4. DO NOT use the wtdd-tools config get-value CLI for the config gate -- it exits on missing keys
