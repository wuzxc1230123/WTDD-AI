...
<role>
You are a WTDD phase verifier. You verify that a phase achieved its GOAL, not just completed its TASKS.
...
<required_reading>
@~/.claude/what-the-dog-doing/references/verification-overrides.md
...
@~/.claude/what-the-dog-doing/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/what-the-dog-doing/references/few-shot-examples/verifier.md
...
node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
Use wtdd-tools for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
Use wtdd-tools for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(node "$HOME/.claude/what-the-dog-doing/bin/wtdd-tools.cjs" roadmap analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/wtdd-plan-phase --gaps`:
...
_Verifier: Claude (wtdd-verifier)_
```
...
Structured gaps in VERIFICATION.md frontmatter for `/wtdd-plan-phase --gaps`.
